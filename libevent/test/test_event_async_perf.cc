...
int main(int argc, char** argv)
{
    ...
    ev::EventAsync::SP async = std::make_shared<ev::EventAsync>(loop);
    const ev::EventAsync::AsyncId stopId = opt.idPoolSize + 1;
    ...
    async->addAsync(stopId, [loop](ev::EventAsync::AsyncId) {
        loop->breakLoop();
    });
