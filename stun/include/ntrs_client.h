...
#define STUN_MAX_IP_LEN     64
#define STUN_MAX_TEXT_LEN   128
#define STUN_MAX_CANDIDATES 8
...
enum {
    STUN_NAT_CLASS_UNKNOWN = 0,
    STUN_NAT_CLASS_OPEN_PUBLIC = 1,
    STUN_NAT_CLASS_FULL_CONE = 2,
    STUN_NAT_CLASS_IP_RESTRICTED = 3,
    STUN_NAT_CLASS_PORT_RESTRICTED = 4,
    STUN_NAT_CLASS_SYMMETRIC = 5,
};

enum {
    STUN_NAT_FLAG_NONE = 0,
    STUN_NAT_FLAG_LOCAL_ADDR_PUBLIC = 1u << 0,
    STUN_NAT_FLAG_UDP_BLOCKED = 1u << 1,
    STUN_NAT_FLAG_PROBE_DEGRADED = 1u << 2,
    STUN_NAT_FLAG_MAPPING_UNSTABLE = 1u << 3,
    STUN_NAT_FLAG_MULTI_EXTERNAL_IP = 1u << 4,
};

enum {
    STUN_MAPPING_UNKNOWN = 0,
    STUN_MAPPING_ENDPOINT_INDEPENDENT = 1,
    STUN_MAPPING_ADDRESS_DEPENDENT = 2,
    STUN_MAPPING_ADDRESS_AND_PORT_DEPENDENT = 3,
    STUN_MAPPING_UNSTABLE = 4,
};

enum {
    STUN_FILTERING_UNKNOWN = 0,
    STUN_FILTERING_ENDPOINT_INDEPENDENT = 1,
    STUN_FILTERING_ADDRESS_DEPENDENT = 2,
    STUN_FILTERING_ADDRESS_AND_PORT_DEPENDENT = 3,
    STUN_FILTERING_BLOCKED = 4,
};

enum {
    STUN_PUNCH_ORDER_UNKNOWN = 0,
    STUN_PUNCH_ORDER_SEND_FIRST = 1,
    STUN_PUNCH_ORDER_WAIT_FIRST = 2,
    STUN_PUNCH_ORDER_SIMULTANEOUS = 3,
};

enum {
    STUN_CONNECT_ROLE_UNKNOWN = 0,
    STUN_CONNECT_ROLE_INITIATOR = 1,
    STUN_CONNECT_ROLE_LISTENER = 2,
};

typedef struct ntrs_nat_info {
    char                      local_ip[STUN_MAX_IP_LEN];
    uint16_t                  local_port;
    char                      srflx_ip[STUN_MAX_IP_LEN];
    uint16_t                  srflx_port;
    char                      srflx_ip_2[STUN_MAX_IP_LEN];
    uint16_t                  srflx_port_2;
    bool                      mapping_stable;
    char                      nat_risk[STUN_MAX_TEXT_LEN];
    bool                      probe1_ok;
    bool                      probe2_ok;
    ...
    char                      nat_type[STUN_MAX_TEXT_LEN];
    bool                      filter_same_ip_diff_port_rx;
    bool                      filter_diff_ip_rx;
} ntrs_nat_info_t;

typedef struct stun_peer_candidate {
    char     ip[STUN_MAX_IP_LEN];
    uint16_t port;
    char     type[STUN_MAX_TEXT_LEN];
} stun_peer_candidate_t;

typedef struct ntrs_session_signal {
    char                      peer_id[STUN_MAX_TEXT_LEN];
    ...
    char                      peer_nat_type[STUN_MAX_TEXT_LEN];
    char                      session_id[STUN_MAX_TEXT_LEN];
    char                      peer_session_token[STUN_MAX_TEXT_LEN];
    ...
    stun_peer_candidate_t     candidates[STUN_MAX_CANDIDATES];
...
typedef enum ntrs_async_request_type {
    STUN_ASYNC_AUTH = 1,
    STUN_ASYNC_REQUEST_PROBE_ENDPOINTS = 2,
    STUN_ASYNC_DETECT_NAT = 3,
    STUN_ASYNC_REGISTER_PEER = 4,
    STUN_ASYNC_UNREGISTER_PEER = 5,
    STUN_ASYNC_CREATE_SESSION = 6,
    STUN_ASYNC_WAIT_FOR_SIGNAL = 7,
    STUN_ASYNC_UDP_HOLE_PUNCH = 8,
    STUN_ASYNC_CONNECT_CONTROL = 9,
    STUN_ASYNC_HEARTBEAT = 10,
...
typedef struct ntrs_async_result {
    ...
    stun_peer_candidate_t     selected_candidate;
    int32_t                   control_fd;
} ntrs_async_result_t;
