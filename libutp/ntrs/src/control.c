...
#define UTP_NTRS_NODE_INSTANCE_WIRE_SIZE      (UTP_NTRS_NODE_ID_SIZE + UTP_NTRS_BOOT_ID_SIZE)
#define UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE    (UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + UTP_NTRS_ENDPOINT_WIRE_SIZE * 2u)
#define UTP_NTRS_REGISTRATION_PAYLOAD_SIZE    (UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 8u + UTP_NTRS_FAMILY_WIRE_SIZE * 2u)
#define UTP_NTRS_REGISTRATION_OK_PAYLOAD_SIZE UTP_NTRS_ENDPOINT_WIRE_SIZE
#define UTP_NTRS_ASSIGNMENT_PAYLOAD_SIZE      (12u + UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE * 2u)
#define UTP_NTRS_HEARTBEAT_PAYLOAD_SIZE       (UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 4u)
...
static bool utp_ntrs_node_id_valid(const uint8_t node_id[UTP_NTRS_NODE_ID_SIZE])
{
    size_t index;
    ...
    for (index = 1u; index < UTP_NTRS_NODE_ID_SIZE; ++index) {
        if (node_id[index] == '\0') {
            terminated = true;
        } else if (terminated) {
            return false;
...
static bool utp_ntrs_node_instance_valid(const utp_ntrs_node_instance_t* instance)
{
    return utp_ntrs_node_id_valid(instance->node_id) &&
           utp_ntrs_bytes_nonzero(instance->boot_id, sizeof(instance->boot_id));
}

static void utp_ntrs_encode_endpoint(uint8_t* data, const utp_ntrs_endpoint_t* endpoint)
{
...
static bool utp_ntrs_decode_endpoint(const uint8_t* data, utp_ntrs_endpoint_t* endpoint)
{
    ...
    return utp_ntrs_family_valid(endpoint->family) &&
           (endpoint->family != (uint8_t)AF_INET || utp_ntrs_bytes_all_zero(endpoint->address + 4u, 12u));
}

static void utp_ntrs_encode_family(uint8_t* data, const utp_ntrs_node_family_t* family)
{
    ...
    utp_ntrs_encode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE, &family->probe_endpoint);
    utp_ntrs_encode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE * 2u, &family->change_port_endpoint);
    utp_ntrs_encode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE * 3u, &family->control_endpoint);
}

static bool utp_ntrs_decode_family(const uint8_t* data, utp_ntrs_node_family_t* family)
{
    *family = (utp_ntrs_node_family_t){0};
    ...
    if (!utp_ntrs_decode_endpoint(data + 4u, &family->public_endpoint) ||
        !utp_ntrs_decode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE, &family->probe_endpoint) ||
        !utp_ntrs_decode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE * 2u, &family->change_port_endpoint) ||
        !utp_ntrs_decode_endpoint(data + 4u + UTP_NTRS_ENDPOINT_WIRE_SIZE * 3u, &family->control_endpoint)) {
        return false;
    }
    return utp_ntrs_family_valid(family->family);
}

static void utp_ntrs_encode_assignment_peer(uint8_t* data, const utp_ntrs_node_instance_t* instance,
                                            const utp_ntrs_endpoint_t* probe, const utp_ntrs_endpoint_t* control)
{
    ...
    utp_ntrs_encode_endpoint(data + sizeof(*instance) + UTP_NTRS_ENDPOINT_WIRE_SIZE, control);
}

static bool utp_ntrs_decode_assignment_peer(const uint8_t* data, utp_ntrs_node_instance_t* instance,
                                            utp_ntrs_endpoint_t* probe, utp_ntrs_endpoint_t* control)
{
    (void)memcpy(instance, data, sizeof(*instance));
    return utp_ntrs_decode_endpoint(data + sizeof(*instance), probe) &&
           utp_ntrs_decode_endpoint(data + sizeof(*instance) + UTP_NTRS_ENDPOINT_WIRE_SIZE, control);
}

static bool utp_ntrs_assignment_peer_valid(const utp_ntrs_node_instance_t* instance, const utp_ntrs_endpoint_t* probe,
                                           const utp_ntrs_endpoint_t* control, uint8_t family)
{
    return utp_ntrs_node_instance_valid(instance) && probe->family == family && control->family == family &&
           probe->port != 0u && control->port != 0u;
}
...
bool utp_ntrs_control_decode_header(const uint8_t* data, size_t length, utp_ntrs_control_header_t* header)
{
    ...
    if (length < UTP_NTRS_CONTROL_HEADER_SIZE || data[0] != UTP_NTRS_CONTROL_VERSION ||
        !utp_ntrs_control_type_valid(data[1]) || data[2] != 0u || data[3] != 0u) {
        return false;
    }
    payload_length = utp_ntrs_read_u32(data + 4u);
    if (payload_length > UTP_NTRS_CONTROL_MAX_MESSAGE_SIZE - UTP_NTRS_CONTROL_HEADER_SIZE ||
...
size_t utp_ntrs_control_encode_header(uint8_t* data, size_t capacity, uint8_t type, uint32_t payload_length)
{
    if (!utp_ntrs_control_type_valid(type) ||
        payload_length > UTP_NTRS_CONTROL_MAX_MESSAGE_SIZE - UTP_NTRS_CONTROL_HEADER_SIZE ||
                                            ...
                                            const utp_ntrs_node_registration_t* registration)
{
    ...
    if (utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_REGISTER,
                                       UTP_NTRS_REGISTRATION_PAYLOAD_SIZE) == 0u) {
        return 0u;
    }
    payload = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    (void)memcpy(payload, &registration->instance, sizeof(registration->instance));
    utp_ntrs_write_u32(payload + sizeof(registration->instance), registration->load);
    utp_ntrs_write_u32(payload + sizeof(registration->instance) + 4u, registration->heartbeat_ms);
    utp_ntrs_encode_family(payload + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 8u, &registration->ipv4);
    utp_ntrs_encode_family(payload + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 8u + UTP_NTRS_FAMILY_WIRE_SIZE,
                           &registration->ipv6);
                                          ...
                                          utp_ntrs_node_registration_t* registration)
{
    ...
    if (!utp_ntrs_control_decode_header(data, length, &header) || header.type != UTP_NTRS_CONTROL_NODE_REGISTER ||
        header.payload_length != UTP_NTRS_REGISTRATION_PAYLOAD_SIZE) {
        return false;
    }
    payload       = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    *registration = (utp_ntrs_node_registration_t){0};
    (void)memcpy(&registration->instance, payload, sizeof(registration->instance));
    registration->load         = utp_ntrs_read_u32(payload + sizeof(registration->instance));
    registration->heartbeat_ms = utp_ntrs_read_u32(payload + sizeof(registration->instance) + 4u);
    return utp_ntrs_node_instance_valid(&registration->instance) && registration->heartbeat_ms != 0u &&
           utp_ntrs_decode_family(payload + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 8u, &registration->ipv4) &&
           utp_ntrs_decode_family(payload + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE + 8u + UTP_NTRS_FAMILY_WIRE_SIZE,
                                  &registration->ipv6) &&
                                               ...
                                               const utp_ntrs_registration_ok_t* registration_ok)
{
    if (registration_ok == NULL || registration_ok->public_endpoint.port != 0u ||
        ...
        utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_REGISTER_OK,
                                       UTP_NTRS_REGISTRATION_OK_PAYLOAD_SIZE) == 0u) {
        return 0u;
    }
    utp_ntrs_encode_endpoint(data + UTP_NTRS_CONTROL_HEADER_SIZE, &registration_ok->public_endpoint);
                                             ...
                                             utp_ntrs_registration_ok_t* registration_ok)
{
    ...
    if (registration_ok == NULL || !utp_ntrs_control_decode_header(data, length, &header) ||
        header.type != UTP_NTRS_CONTROL_NODE_REGISTER_OK ||
        header.payload_length != UTP_NTRS_REGISTRATION_OK_PAYLOAD_SIZE ||
        !utp_ntrs_decode_endpoint(data + UTP_NTRS_CONTROL_HEADER_SIZE, &registration_ok->public_endpoint) ||
...
size_t utp_ntrs_control_encode_assignment(uint8_t* data, size_t capacity, const utp_ntrs_assignment_t* assignment)
{
    ...
    if (utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_ASSIGNMENT,
                                       UTP_NTRS_ASSIGNMENT_PAYLOAD_SIZE) == 0u) {
        return 0u;
    }
    payload    = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    payload[0] = assignment->family;
    payload[1] = (uint8_t)((assignment->has_primary ? 1u : 0u) | (assignment->has_backup ? 2u : 0u));
    payload[2] = 0u;
    payload[3] = 0u;
    utp_ntrs_write_u64(payload + 4u, assignment->version);
    utp_ntrs_encode_assignment_peer(payload + 12u, &assignment->primary, &assignment->primary_probe,
                                    &assignment->primary_control);
    utp_ntrs_encode_assignment_peer(payload + 12u + UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE, &assignment->backup,
                                    &assignment->backup_probe, &assignment->backup_control);
...
bool utp_ntrs_control_decode_assignment(const uint8_t* data, size_t length, utp_ntrs_assignment_t* assignment)
{
    ...
    if (!utp_ntrs_control_decode_header(data, length, &header) || header.type != UTP_NTRS_CONTROL_NODE_ASSIGNMENT ||
        header.payload_length != UTP_NTRS_ASSIGNMENT_PAYLOAD_SIZE) {
        return false;
    }
    payload = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    flags   = payload[1];
    if (!utp_ntrs_family_valid(payload[0]) || (flags & ~3u) != 0u || payload[2] != 0u || payload[3] != 0u ||
    ...
    *assignment             = (utp_ntrs_assignment_t){0};
    ...
    if (assignment->has_primary) {
        if (!utp_ntrs_decode_assignment_peer(payload + 12u, &assignment->primary, &assignment->primary_probe,
                                             &assignment->primary_control) ||
            !utp_ntrs_assignment_peer_valid(&assignment->primary, &assignment->primary_probe,
                                            &assignment->primary_control, assignment->family)) {
            return false;
        }
    } else if (!utp_ntrs_bytes_all_zero(payload + 12u, UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE)) {
        return false;
    }
    if (assignment->has_backup) {
        return utp_ntrs_decode_assignment_peer(payload + 12u + UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE, &assignment->backup,
                                               &assignment->backup_probe, &assignment->backup_control) &&
               utp_ntrs_assignment_peer_valid(&assignment->backup, &assignment->backup_probe,
                                              &assignment->backup_control, assignment->family);
    }
    return utp_ntrs_bytes_all_zero(payload + 12u + UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE,
                                   UTP_NTRS_ASSIGNMENT_PEER_WIRE_SIZE);
}

size_t utp_ntrs_control_encode_heartbeat(uint8_t* data, size_t capacity, const utp_ntrs_node_heartbeat_t* heartbeat)
{
    const size_t message_length = UTP_NTRS_CONTROL_HEADER_SIZE + UTP_NTRS_HEARTBEAT_PAYLOAD_SIZE;

    if (capacity < message_length || utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_HEARTBEAT,
                                                                    UTP_NTRS_HEARTBEAT_PAYLOAD_SIZE) == 0u) {
        return 0u;
    }
    (void)memcpy(data + UTP_NTRS_CONTROL_HEADER_SIZE, &heartbeat->instance, sizeof(heartbeat->instance));
    utp_ntrs_write_u32(data + UTP_NTRS_CONTROL_HEADER_SIZE + sizeof(heartbeat->instance), heartbeat->load);
...
bool utp_ntrs_control_decode_heartbeat(const uint8_t* data, size_t length, utp_ntrs_node_heartbeat_t* heartbeat)
{
    ...
    if (!utp_ntrs_control_decode_header(data, length, &header) || header.type != UTP_NTRS_CONTROL_NODE_HEARTBEAT ||
                                                  ...
                                                  const utp_ntrs_assignment_request_t* request)
{
    ...
    if ((request->failed_roles & ~(UTP_NTRS_ASSIGNMENT_ROLE_PRIMARY | UTP_NTRS_ASSIGNMENT_ROLE_BACKUP)) != 0u ||
        request->failed_roles == 0u || request->assignment_version == 0u || !utp_ntrs_family_valid(request->family) ||
        ((request->failed_roles & UTP_NTRS_ASSIGNMENT_ROLE_PRIMARY) == 0u &&
         !utp_ntrs_bytes_all_zero((const uint8_t*)&request->failed_primary, sizeof(request->failed_primary))) ||
        ((request->failed_roles & UTP_NTRS_ASSIGNMENT_ROLE_BACKUP) == 0u &&
         !utp_ntrs_bytes_all_zero((const uint8_t*)&request->failed_backup, sizeof(request->failed_backup))) ||
        capacity < UTP_NTRS_CONTROL_HEADER_SIZE + UTP_NTRS_ASSIGNMENT_REQUEST_PAYLOAD_SIZE ||
        utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_ASSIGNMENT_REQUEST,
    ...
    (void)memcpy(payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE, &request->failed_primary,
                 sizeof(request->failed_primary));
    (void)memcpy(payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE * 2u, &request->failed_backup,
                 sizeof(request->failed_backup));
    return UTP_NTRS_CONTROL_HEADER_SIZE + UTP_NTRS_ASSIGNMENT_REQUEST_PAYLOAD_SIZE;
}
                                                ...
                                                utp_ntrs_assignment_request_t* request)
{
    ...
    payload = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    if (!utp_ntrs_family_valid(payload[0]) || payload[1] == 0u || utp_ntrs_read_u64(payload + 4u) == 0u ||
        (payload[1] & ~(UTP_NTRS_ASSIGNMENT_ROLE_PRIMARY | UTP_NTRS_ASSIGNMENT_ROLE_BACKUP)) != 0u ||
        payload[2] != 0u || payload[3] != 0u ||
        ((payload[1] & UTP_NTRS_ASSIGNMENT_ROLE_PRIMARY) == 0u &&
         !utp_ntrs_bytes_all_zero(payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE, sizeof(request->failed_primary))) ||
        ((payload[1] & UTP_NTRS_ASSIGNMENT_ROLE_BACKUP) == 0u &&
         !utp_ntrs_bytes_all_zero(payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE * 2u,
                                  sizeof(request->failed_backup)))) {
        return false;
    }
    *request                    = (utp_ntrs_assignment_request_t){0};
    ...
    (void)memcpy(&request->failed_primary, payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE,
                 sizeof(request->failed_primary));
    (void)memcpy(&request->failed_backup, payload + 12u + UTP_NTRS_NODE_INSTANCE_WIRE_SIZE * 2u,
                 sizeof(request->failed_backup));
...
{
    const size_t message_length = UTP_NTRS_CONTROL_HEADER_SIZE + UTP_NTRS_LINK_HELLO_PAYLOAD_SIZE;

    if (capacity < message_length || utp_ntrs_control_encode_header(data, capacity, UTP_NTRS_CONTROL_NODE_LINK_HELLO,
                                                                    UTP_NTRS_LINK_HELLO_PAYLOAD_SIZE) == 0u) {
        return 0u;
    }
    (void)memcpy(data + UTP_NTRS_CONTROL_HEADER_SIZE, &hello->instance, sizeof(hello->instance));
    (void)memcpy(data + UTP_NTRS_CONTROL_HEADER_SIZE + sizeof(hello->instance), hello->initiator_node_id,
                 sizeof(hello->initiator_node_id));
    (void)memcpy(data + UTP_NTRS_CONTROL_HEADER_SIZE + sizeof(hello->instance) + sizeof(hello->initiator_node_id),
...
bool utp_ntrs_control_decode_link_hello(const uint8_t* data, size_t length, utp_ntrs_node_link_hello_t* hello)
{
    ...
    if (!utp_ntrs_control_decode_header(data, length, &header) || header.type != UTP_NTRS_CONTROL_NODE_LINK_HELLO ||
        header.payload_length != UTP_NTRS_LINK_HELLO_PAYLOAD_SIZE) {
        return false;
    }
    payload = data + UTP_NTRS_CONTROL_HEADER_SIZE;
    (void)memcpy(&hello->instance, payload, sizeof(hello->instance));
    (void)memcpy(hello->initiator_node_id, payload + sizeof(hello->instance), sizeof(hello->initiator_node_id));
    (void)memcpy(hello->initiator_nonce, payload + sizeof(hello->instance) + sizeof(hello->initiator_node_id),
                 sizeof(hello->initiator_nonce));
    return utp_ntrs_node_instance_valid(&hello->instance) && utp_ntrs_node_id_valid(hello->initiator_node_id) &&
                                                     ...
                                                     utp_ntrs_forward_filter_response_t* forward)
{
    ...
    *forward            = (utp_ntrs_forward_filter_response_t){0};
    ...
    if (!utp_ntrs_node_instance_valid(&forward->target) ||
        !utp_ntrs_decode_endpoint(payload + UTP_NTRS_FORWARD_CLIENT_OFFSET, &forward->client) ||
