...
static bool utp_ntrs_peer_manager_has_active_remote(
    const utp_ntrs_peer_manager_t *manager, const utp_ntrs_peer_t *excluded,
...
static bool utp_ntrs_peer_message_from_payload(
...
static void utp_ntrs_peer_close_deferred(evutil_socket_t fd, int16_t events,
                                         void *user_data) {
  (void)fd;
  (void)events;
...
static bool utp_ntrs_peer_send(utp_ntrs_peer_t *peer, const uint8_t *message,
                               size_t length) {
...
static bool utp_ntrs_peer_send_header(utp_ntrs_peer_t *peer, uint8_t type) {
  ...
  return utp_ntrs_control_encode_header(message, sizeof(message), type, 0u) !=
             0u &&
...
static bool utp_ntrs_peer_send_hello(utp_ntrs_peer_t *peer) {
  ...
  uint8_t message[UTP_NTRS_CONTROL_HEADER_SIZE + 64u];
  size_t length;

  (void)memcpy(hello.initiator_node_id, peer->initiator_node_id,
               sizeof(hello.initiator_node_id));
  (void)memcpy(hello.initiator_nonce, peer->initiator_nonce,
...
static bool utp_ntrs_peer_matches(const utp_ntrs_peer_t *peer,
                                  const utp_ntrs_node_instance_t *remote) {
  return (peer->hello_received &&
          utp_ntrs_node_instance_equal(&peer->remote, remote)) ||
         (peer->outbound &&
          utp_ntrs_node_instance_equal(&peer->expected_remote, remote));
}

static int32_t utp_ntrs_peer_key_compare(const utp_ntrs_peer_t *left,
                                         const utp_ntrs_peer_t *right) {
  const int32_t node_id_result =
      memcmp(left->initiator_node_id, right->initiator_node_id,
             sizeof(left->initiator_node_id));

  return node_id_result != 0
             ? node_id_result
             : memcmp(left->initiator_nonce, right->initiator_nonce,
                      sizeof(left->initiator_nonce));
}
...
static void utp_ntrs_peer_activate(utp_ntrs_peer_t *peer) {
  ...
  for (current = manager->peers; current != NULL; current = current->next) {
    if (current == peer || !current->active ||
        !utp_ntrs_node_instance_equal(&current->remote, &peer->remote)) {
      continue;
    }
    if (utp_ntrs_peer_key_compare(peer, current) >= 0) {
      peer->suppress_failure = true;
      (void)utp_ntrs_peer_send_header(peer,
                                      UTP_NTRS_CONTROL_NODE_LINK_DUPLICATE);
      (void)utp_ntrs_tls_stream_flush(peer->tls);
      utp_ntrs_peer_schedule_close(peer);
      return;
    }
    current->suppress_failure = true;
    (void)utp_ntrs_peer_send_header(current,
                                    UTP_NTRS_CONTROL_NODE_LINK_DUPLICATE);
...
static void utp_ntrs_peer_on_message(void *user_data, uint8_t type,
  ...
  if (!utp_ntrs_peer_message_from_payload(type, payload, payload_length,
                                          message, &length)) {
    utp_ntrs_peer_schedule_close(peer);
    return;
  }
  if (!peer->hello_received) {
    ...
    if (type != UTP_NTRS_CONTROL_NODE_LINK_HELLO ||
        !utp_ntrs_control_decode_link_hello(message, length, &hello) ||
        utp_ntrs_node_instance_equal(&hello.instance, &peer->manager->local) ||
        (peer->outbound && !utp_ntrs_node_instance_equal(
                               &hello.instance, &peer->expected_remote)) ||
        (!peer->outbound &&
         memcmp(hello.initiator_node_id, hello.instance.node_id,
                sizeof(hello.initiator_node_id)) != 0) ||
        (peer->outbound &&
         memcmp(hello.initiator_node_id, peer->manager->local.node_id,
                sizeof(hello.initiator_node_id)) != 0)) {
      utp_ntrs_peer_schedule_close(peer);
      return;
    }
    ...
    if (!peer->outbound) {
      (void)memcpy(peer->initiator_node_id, hello.initiator_node_id,
                   sizeof(peer->initiator_node_id));
      (void)memcpy(peer->initiator_nonce, hello.initiator_nonce,
                   sizeof(peer->initiator_nonce));
      if (!utp_ntrs_peer_send_hello(peer)) {
        return;
      }
    } else if (memcmp(peer->initiator_node_id, hello.initiator_node_id,
                      sizeof(peer->initiator_node_id)) != 0 ||
               memcmp(peer->initiator_nonce, hello.initiator_nonce,
                      sizeof(peer->initiator_nonce)) != 0) {
      utp_ntrs_peer_schedule_close(peer);
      return;
    }
  ...
  if (type == UTP_NTRS_CONTROL_NAT_FORWARD_FILTER_RSP) {
    ...
    if (!peer->active ||
        !utp_ntrs_control_decode_forward_filter_response(message, length,
                                                         &forward) ||
...
static void utp_ntrs_peer_on_plaintext(void *user_data, const uint8_t *data,
                                       size_t length) {
  utp_ntrs_peer_t *const peer = user_data;

  if (!utp_ntrs_control_stream_feed(&peer->control, data, length,
                                    utp_ntrs_peer_on_message, peer)) {
    utp_ntrs_peer_schedule_close(peer);
  }
}

static void utp_ntrs_peer_on_closed(void *user_data) {
  utp_ntrs_peer_schedule_close(user_data);
}
...
static void utp_ntrs_peer_destroy(utp_ntrs_peer_t *peer) {
  ...
  const utp_ntrs_node_instance_t *const failed_remote =
      peer->active ? &peer->remote : &peer->expected_remote;

  while (*link != peer) {
    link = &(*link)->next;
  }
  *link = peer->next;
  if (!peer->suppress_failure && peer->manager->on_failed != NULL &&
      (peer->active || peer->outbound) &&
      !utp_ntrs_peer_manager_has_active_remote(peer->manager, peer,
                                               failed_remote)) {
...
static utp_ntrs_peer_t *utp_ntrs_peer_new(utp_ntrs_peer_manager_t *manager,
                                          int32_t fd, bool outbound) {
  ...
  peer->tls = utp_ntrs_tls_stream_new(
      manager->base, fd, outbound ? manager->client_tls : manager->server_tls,
      !outbound, &callbacks, peer);
  peer->close_event = event_new(manager->base, -1, EV_TIMEOUT,
                                utp_ntrs_peer_close_deferred, peer);
...
static void utp_ntrs_peer_on_accept(struct evconnlistener *listener,
                                    evutil_socket_t fd,
...
utp_ntrs_peer_manager_start(const utp_ntrs_peer_manager_options_t *options) {
  ...
  if (manager == NULL || !utp_ntrs_endpoint_to_sockaddr(
                             &options->listen, &address, &address_length)) {
    free(manager);
    return NULL;
  }
...
bool utp_ntrs_peer_manager_connect(utp_ntrs_peer_manager_t *manager,
  ...
  (void)memcpy(peer->initiator_node_id, manager->local.node_id,
               sizeof(peer->initiator_node_id));
  if (getrandom(peer->initiator_nonce, sizeof(peer->initiator_nonce), 0u) !=
          (ssize_t)sizeof(peer->initiator_nonce) ||
      bufferevent_socket_connect(utp_ntrs_tls_stream_transport(peer->tls),
...
bool utp_ntrs_peer_manager_send_forward(
    utp_ntrs_peer_manager_t *manager, const utp_ntrs_node_instance_t *target,
    const utp_ntrs_forward_filter_response_t *forward) {
  utp_ntrs_peer_t *peer;
  uint8_t message[UTP_NTRS_CONTROL_HEADER_SIZE + 83u];
  size_t length;

  for (peer = manager->peers; peer != NULL; peer = peer->next) {
    if (peer->active && !peer->close_scheduled &&
        utp_ntrs_node_instance_equal(&peer->remote, target)) {
