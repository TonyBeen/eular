...
typedef void (*utp_ntrs_tls_plaintext_fn)(void* user_data, const uint8_t* data, size_t length);
...
SSL_CTX*               utp_ntrs_tls_server_context_new(const char* certificate_path, const char* private_key_path);
 ...
 */
utp_ntrs_tls_stream_t* utp_ntrs_tls_stream_new(event_base* base, int32_t fd, SSL_CTX* context, bool server,
                                               const utp_ntrs_tls_callbacks_t* callbacks, void* user_data);
...
bool                   utp_ntrs_tls_stream_send(utp_ntrs_tls_stream_t* stream, const uint8_t* data, size_t length);
