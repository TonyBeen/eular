...
void utp_ntrs_control_stream_init(utp_ntrs_control_stream_t *stream) {
  *stream = (utp_ntrs_control_stream_t){0};
}

bool utp_ntrs_control_stream_feed(utp_ntrs_control_stream_t *stream,
                                  const uint8_t *data, size_t length,
                                  utp_ntrs_control_message_fn callback,
                                  void *user_data) {
  while (length != 0u) {
    size_t copy_length;

    if (stream->expected_length == 0u &&
        stream->used < UTP_NTRS_CONTROL_HEADER_SIZE) {
      ...
      if (stream->buffer[0] != UTP_NTRS_CONTROL_VERSION ||
          !utp_ntrs_control_type_valid(stream->buffer[1]) ||
          stream->buffer[2] != 0u || stream->buffer[3] != 0u) {
        return false;
      }
      stream->expected_length = UTP_NTRS_CONTROL_HEADER_SIZE +
                                utp_ntrs_control_read_u32(stream->buffer + 4u);
      if (stream->expected_length > UTP_NTRS_CONTROL_MAX_MESSAGE_SIZE) {
        return false;
      }
      if (stream->used == stream->expected_length) {
        callback(user_data, stream->buffer[1],
                 stream->buffer + UTP_NTRS_CONTROL_HEADER_SIZE, 0u);
    ...
    if (stream->used == stream->expected_length) {
      callback(user_data, stream->buffer[1],
               stream->buffer + UTP_NTRS_CONTROL_HEADER_SIZE,
               stream->expected_length - UTP_NTRS_CONTROL_HEADER_SIZE);
      utp_ntrs_control_stream_init(stream);
    }
  }
