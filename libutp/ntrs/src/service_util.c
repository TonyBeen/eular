...
bool utp_ntrs_endpoint_parse(const char* text, utp_ntrs_endpoint_t* endpoint)
{
    ...
    *endpoint = (utp_ntrs_endpoint_t){0};
    if (text[0] == '[') {
        ...
        port_text      = closing + 2;
        if (address_length == 0u || address_length >= sizeof(address) || !utp_ntrs_parse_port(port_text, &port)) {
            return false;
        }
...
bool utp_ntrs_endpoint_resolve_for_family(const char* text, int32_t family, utp_ntrs_endpoint_t* endpoint)
{
    ...
    if (separator == NULL || separator == text || separator[1] == '\0' || !utp_ntrs_parse_port(separator + 1, &port)) {
        return false;
    }
    ...
    if (host_length == 0u || host_length >= sizeof(host) ||
        (size_t)snprintf(service, sizeof(service), "%u", (uint32_t)port) >= sizeof(service)) {
        return false;
    }
    ...
    for (result = addresses; result != NULL; result = result->ai_next) {
        if (family == AF_INET && result->ai_family == AF_INET) {
            const struct sockaddr_in* const address = (const struct sockaddr_in*)result->ai_addr;

            *endpoint = (utp_ntrs_endpoint_t){
                .family = (uint8_t)AF_INET,
                .port   = ntohs(address->sin_port),
            };
            (void)memcpy(endpoint->address, &address->sin_addr, sizeof(address->sin_addr));
            freeaddrinfo(addresses);
            return true;
        }
        if (family == AF_INET6 && result->ai_family == AF_INET6) {
            const struct sockaddr_in6* const address = (const struct sockaddr_in6*)result->ai_addr;
            ...
            *endpoint = (utp_ntrs_endpoint_t){
            ...
            (void)memcpy(endpoint->address, &address->sin6_addr, sizeof(address->sin6_addr));
            freeaddrinfo(addresses);
            return true;
        }
    }
...
bool utp_ntrs_socket_bind_interface(int32_t fd, const char* interface_name)
{
    ...
    return interface_name == NULL ||
           (length != 0u && length < (size_t)IFNAMSIZ &&
            setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, interface_name, (socklen_t)(length + 1u)) == 0);
}
                                   ...
                                   socklen_t* length)
{
    *storage = (struct sockaddr_storage){0};
    if (endpoint->family == (uint8_t)AF_INET) {
        ...
        (void)memcpy(&address->sin_addr, endpoint->address, sizeof(address->sin_addr));
    ...
    if (endpoint->family == (uint8_t)AF_INET6) {
        ...
        (void)memcpy(&address->sin6_addr, endpoint->address, sizeof(address->sin6_addr));
        *length = (socklen_t)sizeof(*address);
        return true;
    }
    return false;
...
const char* utp_ntrs_endpoint_format(const utp_ntrs_endpoint_t* endpoint, char* buffer, size_t capacity)
{
    ...
    if (endpoint->family == (uint8_t)AF_INET) {
        if (inet_ntop(AF_INET, endpoint->address, address, sizeof(address)) == NULL ||
            (size_t)snprintf(buffer, capacity, "%s:%u", address, (uint32_t)endpoint->port) >= capacity) {
            return NULL;
        }
        return buffer;
    }
    if (endpoint->family == (uint8_t)AF_INET6) {
        if (inet_ntop(AF_INET6, endpoint->address, address, sizeof(address)) == NULL ||
            (size_t)snprintf(buffer, capacity, "[%s]:%u", address, (uint32_t)endpoint->port) >= capacity) {
            return NULL;
        }
        return buffer;
...
const char* utp_ntrs_node_instance_format(const utp_ntrs_node_instance_t* instance, char* buffer, size_t capacity)
{
    ...
    if (capacity < node_id_length + 1u + UTP_NTRS_BOOT_ID_SIZE * 2u + 1u) {
        return NULL;
    }
    (void)memcpy(buffer, instance->node_id, node_id_length);
    buffer[node_id_length] = ':';
    for (index = 0u; index < UTP_NTRS_BOOT_ID_SIZE; ++index) {
        const size_t offset = node_id_length + 1u + index * 2u;

        buffer[offset]      = digits[instance->boot_id[index] >> 4u];
        buffer[offset + 1u] = digits[instance->boot_id[index] & 0x0fu];
    }
    buffer[node_id_length + 1u + UTP_NTRS_BOOT_ID_SIZE * 2u] = '\0';
    return buffer;
}
