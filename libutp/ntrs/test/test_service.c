...
static utp_ntrs_endpoint_t test_endpoint(uint8_t family, uint16_t port, uint8_t last_octet)
{
...
static void test_service_endpoint_resolution(void)
{
    ...
    assert(endpoint.port == 24000u);
    assert(endpoint.family == (uint8_t)AF_INET);
    assert(utp_ntrs_endpoint_resolve_for_family("[::1]:24000", AF_INET6, &endpoint));
    assert(endpoint.family == (uint8_t)AF_INET6);
    assert(endpoint.port == 24000u);
    assert(!utp_ntrs_endpoint_resolve_for_family("127.0.0.1:24000", AF_INET6, &endpoint));
    assert(utp_ntrs_endpoint_from_sockaddr(&endpoint, (const struct sockaddr*)&source, (socklen_t)sizeof(source)));
    assert(endpoint.family == (uint8_t)AF_INET);
...
static void test_node_instance_format(void)
{
...
static utp_ntrs_node_registration_t test_registration(uint8_t id, uint8_t boot, uint8_t public_ip, uint32_t load)
{
    ...
    registration.ipv4.public_endpoint      = test_endpoint((uint8_t)AF_INET, 0u, public_ip);
    registration.ipv4.probe_endpoint       = test_endpoint((uint8_t)AF_INET, (uint16_t)(4000u + id), public_ip);
    registration.ipv4.change_port_endpoint = test_endpoint((uint8_t)AF_INET, (uint16_t)(5000u + id), public_ip);
    registration.ipv4.control_endpoint     = test_endpoint((uint8_t)AF_INET, (uint16_t)(6000u + id), public_ip);
    return registration;
}

static void test_node_id_wire_boundaries(void)
{
    static const char node_id[] =
        "a7k2m9q4v1x6r3t8"
        "a7k2m9q4v1x6r3t8"
        ...
        "a7k2m9q4v1x6r3t8"
        "a7k2m9q4v1x6r3t8";
    utp_ntrs_node_registration_t registration = test_registration(1u, 2u, 3u, 4u);
    utp_ntrs_node_registration_t decoded      = {0};
    uint8_t                      message[512] = {0};
    size_t                       length;

    assert(sizeof(node_id) - 1u == UTP_NTRS_NODE_ID_SIZE);
    (void)memcpy(registration.instance.node_id, node_id, UTP_NTRS_NODE_ID_SIZE);
    ...
    assert(memcmp(decoded.instance.node_id, node_id, UTP_NTRS_NODE_ID_SIZE) == 0);

    message[UTP_NTRS_CONTROL_HEADER_SIZE + 20u] = 0u;
    assert(!utp_ntrs_control_decode_registration(message, length, &decoded));
}

static void test_control_codec(void)
{
    const utp_ntrs_node_registration_t registration = test_registration(1u, 9u, 1u, 7u);
    utp_ntrs_node_registration_t       decoded      = {0};
    ...
    message[2] = 1u;
    assert(!utp_ntrs_control_decode_registration(message, length, &decoded));
    length = utp_ntrs_control_encode_assignment(message, sizeof(message), &assignment);
    assert(length == 384u);
    assert(utp_ntrs_control_decode_assignment(message, length, &decoded_assignment));
    ...
    assert(length == 156u);
    assert(utp_ntrs_control_decode_heartbeat(message, length, &decoded_heartbeat));
    ...
    assert(length == 452u);
    assert(utp_ntrs_control_decode_assignment_request(message, length, &decoded_request));
    ...
    assert(memcmp(decoded_forward.client.address, forward.client.address, sizeof(forward.client.address)) == 0);
    assert(memcmp(decoded_forward.token, forward.token, sizeof(forward.token)) == 0);
}
...
static void test_control_stream_message(void* user_data, uint8_t type, const uint8_t* payload, uint32_t payload_length)
{
...
{
    const utp_ntrs_node_registration_t registration = test_registration(4u, 1u, 4u, 1u);
    ...
    assert(utp_ntrs_control_stream_feed(&stream, message, 3u, test_control_stream_message, &callback));
    assert(callback.calls == 0u);
    assert(utp_ntrs_control_stream_feed(&stream, message + 3u, length - 3u, test_control_stream_message, &callback));
    assert(callback.calls == 1u);
    assert(callback.last_type == UTP_NTRS_CONTROL_NODE_REGISTER);
    assert(callback.last_size == 312u);
    (void)memcpy(combined, message, length);
    (void)memcpy(combined + length, message, length);
    assert(utp_ntrs_control_stream_feed(&stream, combined, length * 2u, test_control_stream_message, &callback));
    assert(callback.calls == 3u);
    message[0] = UTP_NTRS_CONTROL_VERSION + 1u;
    utp_ntrs_control_stream_init(&stream);
    assert(!utp_ntrs_control_stream_feed(&stream, message, UTP_NTRS_CONTROL_HEADER_SIZE, test_control_stream_message,
                                         &callback));
}
...
static void test_registration_ok(void)
{
    ...
    length = utp_ntrs_control_encode_registration_ok(message, sizeof(message), &expected);
...
static void test_plain_stream_ready(void* user_data) { ++((test_plain_stream_callback_t*)user_data)->ready_count; }

static void test_plain_stream_data(void* user_data, const uint8_t* data, size_t length)
{
...
static void test_plain_tcp_stream(void)
{
    ...
    assert((server = utp_ntrs_tls_stream_new(base, sockets[0], NULL, true, &callbacks, &server_callback)) != NULL);
    assert(server_callback.ready_count == 1u);
    assert((client = utp_ntrs_tls_stream_new(base, sockets[1], NULL, false, &callbacks, &client_callback)) != NULL);
...
static void test_hub_assignment(void)
{
    ...
    assert(utp_ntrs_hub_get_assignment(&hub, &node_a.instance, (uint8_t)AF_INET, &assignment));
...
static void test_hub_reregistration_preserves_healthy_assignment(void)
{
    ...
    assert(utp_ntrs_hub_get_assignment(&hub, &node_c.instance, (uint8_t)AF_INET, &before));
    assert(before.has_primary);
    assert(before.has_backup);
    node_c.load = 100u;
    assert(utp_ntrs_hub_register(&hub, &node_c, 101u, NULL));
    assert(utp_ntrs_hub_get_assignment(&hub, &node_c.instance, (uint8_t)AF_INET, &after));
...
static void test_link_deduplication(void)
{
    ...
    assert(utp_ntrs_node_link_consider(&link, &remote, first_id, nonce, &replaced));
    assert(!replaced);
    assert(utp_ntrs_node_link_consider(&link, &remote, second_id, nonce, &replaced));
    assert(replaced);
    assert(!utp_ntrs_node_link_consider(&link, &remote, first_id, nonce, &replaced));
}
...
static uint16_t test_response_origin_port(const uint8_t* response, size_t response_length)
{
    ...
    return (uint16_t)(((uint16_t)response[offset + 6u] << 8u) | response[offset + 7u]);
}
...
static uint16_t test_allocate_loopback_port(void)
{
    ...
    assert(socket_fd >= 0);
    assert(bind(socket_fd, (const struct sockaddr*)&address, address_length) == 0);
    assert(getsockname(socket_fd, (struct sockaddr*)&address, &address_length) == 0);
...
static void test_udp_worker_ipv6_round_trip(void)
{
    ...
    utp_ntrs_udp_server_options_t options     = {
        ...
        .change_port_endpoint = test_endpoint((uint8_t)AF_INET6, change_port, 1u),
    };
...
static void test_udp_worker_source_rate_shared_by_ip(void)
{
    ...
    for (index = 0u; index < 4u; ++index) {
        if ((poll_fds[index].revents & POLLIN) != 0) {
            assert(recvfrom(clients[index], response, sizeof(response), 0, NULL, NULL) > 0);
            ++response_count;
        }
...
int main(void)
{
    ...
    test_node_instance_format();
    test_node_id_wire_boundaries();
    test_control_codec();
    test_control_stream();
