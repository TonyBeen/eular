...
public:
    ...
    bool start(const utp_ntrs_endpoint_t& endpoint, const char* interface_name);
...
private:
    ...
    bool sendDatagram(const sockaddr_storage& peer, socklen_t peer_length, const uint8_t* packet, size_t packet_length);
    bool sendFrames(const sockaddr_storage& peer, socklen_t peer_length, const OutgoingFrame* frames,
    ...
    bool sendMessage(const sockaddr_storage& peer, socklen_t peer_length, uint8_t message_type, const uint8_t* body,
                     size_t body_length);
    ...
    void handlePing(const sockaddr_storage& peer, socklen_t peer_length, const utp_ntrs_endpoint_t& observed,
                    const utp_packet_header_t& header, const utp_frame_rendezvous_t& frame);
    ...
    std::deque<OutgoingDatagram>                       output_queue_;
...
NtrsServer::~NtrsServer()
{
    if (write_event_ != NULL) event_free(write_event_);
    if (timer_event_ != NULL) event_free(timer_event_);
    if (read_event_ != NULL) event_free(read_event_);
...
static bool address_matches_endpoint_ip(const utp_address_t& address, const utp_ntrs_endpoint_t& endpoint)
{
    const size_t  address_length  = address.family == UTP_ADDRESS_FAMILY_IPV4   ? 4u
                                    : address.family == UTP_ADDRESS_FAMILY_IPV6 ? 16u
                                                                                : 0u;
    const uint8_t endpoint_family = endpoint.family == AF_INET    ? UTP_ADDRESS_FAMILY_IPV4
                                ...
                                const utp_ntrs_endpoint_t& observed_endpoint)
{
    ...
    if (plan == NULL || local_port == 0u || local_candidate_count > UTP_RENDEZVOUS_MAX_LOCAL_CANDIDATES ||
    ...
    *plan                       = {};
    ...
    if (!append_public_candidate(plan, observed_public_endpoint)) return false;
...
bool NtrsServer::sendDatagram(const sockaddr_storage& peer, socklen_t peer_length, const uint8_t* packet,
                              size_t packet_length)
{
    if (fatal_socket_error_ || packet == NULL || packet_length == 0u || packet_length > UTP_PACKET_MTU_FLOOR)
        return false;
    if (output_queue_.empty()) {
        ssize_t sent_length;
        for (;;) {
            sent_length = sendto(fd_, packet, packet_length, 0, reinterpret_cast<const sockaddr*>(&peer), peer_length);
    ...
    if (output_queue_.size() >= k_output_queue_capacity || write_event_ == NULL) return false;
    ...
    output_queue_.push_back(datagram);
    if (!write_event_active_) {
        if (event_add(write_event_, NULL) != 0) {
            output_queue_.pop_back();
            return false;
        }
        write_event_active_ = true;
    }
    return true;
}

bool NtrsServer::sendFrames(const sockaddr_storage& peer, socklen_t peer_length, const OutgoingFrame* frames,
                            size_t frame_count, uint64_t* packet_number,
                            std::array<uint8_t, UTP_PACKET_MTU_FLOOR>* encoded_packet, size_t* encoded_packet_length)
{
    ...
    return sendDatagram(peer, peer_length, packet, offset);
}

bool NtrsServer::sendMessage(const sockaddr_storage& peer, socklen_t peer_length, uint8_t message_type,
                             const uint8_t* body, size_t body_length)
{
    const OutgoingFrame frame = {body, body_length, message_type};

    return sendFrames(peer, peer_length, &frame, 1u, NULL);
}
...
void NtrsServer::retryPendingRendezvous(uint64_t now_ms)
{
    for (std::unordered_map<std::string, PendingRendezvous>::iterator entry = pending_rendezvous_.begin();
        ...
        if (transaction.forward_delivered || transaction.forward_retries_remaining == 0u ||
            ...
            continue;
        if (!sendDatagram(transaction.target_socket_address, transaction.target_socket_length,
                                ...
                                const utp_ntrs_endpoint_t& observed, const utp_frame_rendezvous_t& frame)
{
    ...
    if (utp_rendezvous_registered_encode(body, sizeof(body), &reply, &body_length) == UTP_INTERNAL_ERROR_OK)
        (void)sendMessage(peer, peer_length, UTP_RENDEZVOUS_MESSAGE_REGISTERED, body, body_length);
...
void NtrsServer::handlePing(const sockaddr_storage& peer, socklen_t peer_length, const utp_ntrs_endpoint_t& observed,
                            const utp_packet_header_t& header, const utp_frame_rendezvous_t& frame)
{
    ...
    for (std::unordered_map<std::string, Registration>::iterator entry = registrations_.begin();
        ...
        if (utp_rendezvous_pong_encode(body, sizeof(body), &pong) == UTP_INTERNAL_ERROR_OK)
            (void)sendMessage(peer, peer_length, UTP_RENDEZVOUS_MESSAGE_PONG, body, sizeof(body));
        return;
    }
}
                                     ...
                                     const utp_ntrs_endpoint_t& observed, const utp_frame_rendezvous_t& frame)
{
    ...
    for (std::unordered_map<std::string, Registration>::iterator entry = registrations_.begin();
        ...
        (void)sendMessage(peer, peer_length, UTP_RENDEZVOUS_MESSAGE_ADDRESS_UPDATED, body, body_length);
                               ...
                               const utp_frame_rendezvous_t& frame)
{
    ...
    if (pending != pending_rendezvous_.end()) {
        if (endpoints_equal(pending->second.source_endpoint, observed))
            (void)sendMessage(peer, peer_length, UTP_RENDEZVOUS_MESSAGE_REDIRECT, pending->second.redirect_body.data(),
    ...
    if (!sendMessage(peer, peer_length, UTP_RENDEZVOUS_MESSAGE_REDIRECT, transaction.redirect_body.data(),
    ...
    if (!sendFrames(target_socket_address, target_socket_length, frames, 2u, &transaction.forward_packet_number,
                    &transaction.forward_packet, &transaction.forward_packet_length))
        return;
...
void NtrsServer::OnReadEvent(evutil_socket_t, short, void* user_data) { static_cast<NtrsServer*>(user_data)->onRead(); }

void NtrsServer::onRead()
{
    for (;;) {
        uint8_t          packet[UTP_PACKET_MTU_FLOOR];
        sockaddr_storage peer        = {};
        ...
        const ssize_t    count =
            recvfrom(fd_, packet, sizeof(packet), 0, reinterpret_cast<sockaddr*>(&peer), &peer_length);
        utp_packet_header_t header   = {};
        ...
        if (header.type == UTP_PACKET_TYPE_RENDEZVOUS) {
            ...
            if (frame.message_type == UTP_RENDEZVOUS_MESSAGE_REGISTER)
                ...
                handlePing(peer, peer_length, observed, header, frame);
        ...
        } else if (header.type == UTP_PACKET_TYPE_INITIAL || header.type == UTP_PACKET_TYPE_0RTT) {
            utp_frame_rendezvous_t frame        = {};
...
{
    static_cast<NtrsServer*>(user_data)->onWrite();
}

void NtrsServer::onWrite()
{
    while (!output_queue_.empty()) {
        const OutgoingDatagram& datagram = output_queue_.front();
        ssize_t                 sent_length;
        do {
            sent_length = sendto(fd_, datagram.packet.data(), datagram.packet_length, 0,
...
bool NtrsServer::start(const utp_ntrs_endpoint_t& endpoint, const char* interface_name)
{
    sockaddr_storage socket_address = {};
    socklen_t        socket_length  = 0u;

    if (!utp_ntrs_endpoint_to_sockaddr(&endpoint, &socket_address, &socket_length) ||
        (fd_ = socket(endpoint.family, SOCK_DGRAM, 0)) < 0 || !utp_ntrs_socket_bind_interface(fd_, interface_name) ||
        bind(fd_, reinterpret_cast<const sockaddr*>(&socket_address), socket_length) != 0 ||
        evutil_make_socket_nonblocking(fd_) != 0 || (base_ = event_base_new()) == NULL ||
        (read_event_ = event_new(base_, fd_, EV_READ | EV_PERSIST, OnReadEvent, this)) == NULL ||
        (timer_event_ = event_new(base_, -1, EV_PERSIST, OnTimerEvent, this)) == NULL ||
        (write_event_ = event_new(base_, fd_, EV_WRITE | EV_PERSIST, OnWriteEvent, this)) == NULL ||
        event_add(read_event_, NULL) != 0)
        return false;
    return true;
}
...
int main(int argc, char** argv)
{
    ...
    if (!utp_ntrs_endpoint_parse(bind_endpoint_text.c_str(), &endpoint) ||
        !server.start(endpoint, interface_name.empty() ? NULL : interface_name.c_str()))
        return 1;
