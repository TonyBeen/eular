...
static int parse_endpoint(const char* spec, struct sockaddr_storage* sa, socklen_t* salen)
{
    char             host[256];
    char             port[32];
...
static int ensure_socket(struct prog* prog, const struct sockaddr_storage* bind_sa, socklen_t bind_len)
{
    int yes = 1;

    prog->sockfd = socket(bind_sa->ss_family, SOCK_DGRAM, 0);
    if (prog->sockfd < 0) return -1;
    if (setsockopt(prog->sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) != 0) return -1;
    if (bind(prog->sockfd, (const struct sockaddr*)bind_sa, bind_len) != 0) return -1;
    if (set_nonblocking(prog->sockfd) != 0) return -1;
    return 0;
}

static int packets_out(void* ctx, const struct lsquic_out_spec* out_spec, unsigned n_packets_out)
{
               ...
               void* stream_if_ctx)
{
    ...
    if (flags & LSENG_SERVER) prog->prog_api.ea_get_ssl_ctx = get_ssl_ctx;
    if (!s_global_inited) {
...
static int prep_bind_sa(struct prog* prog, struct sockaddr_storage* sa, socklen_t* salen)
{
...
int prog_prep(struct prog* prog)
{
    ...
    if (prep_bind_sa(prog, &bind_sa, &bind_len) != 0) return -1;

    if (ensure_socket(prog, &bind_sa, bind_len) != 0) return -1;

    prog->local_sa_len = sizeof(prog->local_sa);
    if (getsockname(prog->sockfd, (struct sockaddr*)&prog->local_sa, &prog->local_sa_len) != 0) return -1;
...
int prog_connect(struct prog* prog, const struct sockaddr* local_sa, unsigned short base_plpmtu)
{
    ...
    if (!prog->peer_spec || !prog->engine) return -1;
    if (parse_endpoint(prog->peer_spec, &prog->peer_sa, &prog->peer_sa_len) != 0) return -1;
    prog->has_peer_sa = 1;

    if (!lsquic_engine_connect(prog->engine, N_LSQVER, l_sa, (const struct sockaddr*)&prog->peer_sa, prog, NULL,
                               prog->hostname ? prog->hostname : "localhost",
                               base_plpmtu ? base_plpmtu : prog->prog_settings.es_base_plpmtu, NULL, 0, NULL, 0))
        return -1;
    (void)l_len;
    return 0;
}

static void pump_engine(struct prog* prog)
{
    if (!prog->engine) return;
    lsquic_engine_process_conns(prog->engine);
    while (lsquic_engine_has_unsent_packets(prog->engine)) lsquic_engine_send_unsent_packets(prog->engine);
}
    ...
    while (prog->running) {
        ...
        if (rv > 0 && FD_ISSET(prog->sockfd, &readfds)) {
            for (;;) {
                ...
                lsquic_engine_packet_in(prog->engine, buf, (size_t)nr, (const struct sockaddr*)&prog->local_sa,
...
void prog_stop(struct prog* prog) { prog->running = 0; }
