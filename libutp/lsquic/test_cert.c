                       ...
                       unsigned int inlen, void* arg)
{
    ...
    rv = SSL_select_next_proto((unsigned char**)out, outlen, in, inlen, (unsigned char*)s_alpn, strlen(s_alpn));
    return rv == OPENSSL_NPN_NEGOTIATED ? SSL_TLSEXT_ERR_OK : SSL_TLSEXT_ERR_ALERT_FATAL;
}
...
static SSL_CTX* create_server_ctx(void)
{
    ...
    if (1 != SSL_CTX_use_certificate(ctx, cert)) goto err;
    if (1 != SSL_CTX_use_PrivateKey(ctx, pkey)) goto err;
    if (1 != SSL_CTX_check_private_key(ctx)) goto err;
...
{
    (void)sni;
    if (!s_server_ctx) s_server_ctx = create_server_ctx();
    return s_server_ctx;
}
...
{
    if (ctx) SSL_CTX_free(ctx);
    if (ctx == s_server_ctx) s_server_ctx = NULL;
}
