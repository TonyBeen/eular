...
static int
append_bytes(char **buf, size_t *len, size_t *cap, const void *data, size_t data_len)
{
    char *nbuf;
    size_t new_cap;
...
static int
queue_line(struct server_stream_ctx *st, const char *line)
{
    return append_bytes(&st->outbox, &st->outbox_len, &st->outbox_cap,
                        line, strlen(line));
}

static int
parse_upload_header(const char *line, uint64_t *expected_bytes)
{
...
static int
finalize_hash(struct server_stream_ctx *st)
{
...
static void
mark_failed(struct server_stream_ctx *st, const char *reason)
{
    char line[256];
...
static void
maybe_finalize_done(struct server_stream_ctx *st)
{
    char line[256];
...
static void
process_payload_chunk(struct server_stream_ctx *st,
...
static int
consume_read_buf(struct server_stream_ctx *st, const unsigned char *data, size_t len)
{
    ...
    while (left > 0 && !st->failed)
    {
        if (st->phase == PHASE_READ_HEADER)
        {
            ...
            if (!lf)
            {
                if (0 != append_bytes(&st->header_buf, &st->header_len, &st->header_cap,
                                      chunk, left))
                    return -1;
                return 0;
            }
            ...
            if (st->header_len > 0 && st->header_buf[st->header_len - 1] == '\r')
                --st->header_len;
            if (0 != append_bytes(&st->header_buf, &st->header_len, &st->header_cap, "", 1))
                return -1;
            line = st->header_buf;
...
on_write(lsquic_stream_t *stream, lsquic_stream_ctx_t *st_h)
{
    ...
    {
        nw = lsquic_stream_write(stream, st->outbox + st->outbox_off,
                                 st->outbox_len - st->outbox_off);
        if (nw > 0)
...
{
    fprintf(stderr,
        "Usage: %s [--bind-ip IP] [--bind-port PORT] [--silent]\n",
        prog);
}
...
main(int argc, char **argv)
{
    ...
    if (!quiet)
        fprintf(stderr, "[server] listening on %s:%hu\n", bind_ip, bind_port);
    prog_run(&prog);
    prog_cleanup(&prog);
    return 0;
}
