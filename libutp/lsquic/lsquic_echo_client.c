...
static int finalize_local_hash(struct client_app_ctx* ctx)
{
    ...
    if (ctx->hash_finalized) return 0;
    if (!ctx->hash_valid) return -1;
...
static void print_final_result(struct client_app_ctx* ctx, const char* reason)
{
    int pass;

    if (ctx->final_printed) return;

    ctx->final_printed = 1;
    if (!ctx->hash_finalized) (void)finalize_local_hash(ctx);

    pass = ctx->done_received && !ctx->connect_failed && ctx->sent_bytes == ctx->total_bytes &&
           ctx->done_bytes == ctx->total_bytes && 0 == strcmp(ctx->local_hash, ctx->server_hash);
    ctx->result = pass ? 0 : 1;
    ...
    fprintf(stderr,
            ...
            pass ? "PASS" : "FAIL", reason, ctx->sent_bytes, ctx->done_bytes,
            ctx->connected_ms ? ctx->connected_ms - ctx->start_ms : 0,
            ctx->upload_done_ms ? ctx->upload_done_ms - ctx->start_ms : 0,
            ctx->done_ms ? ctx->done_ms - ctx->start_ms : 0, now_ms() - ctx->start_ms,
            ctx->local_hash[0] ? ctx->local_hash : "<pending>", ctx->server_hash[0] ? ctx->server_hash : "<pending>",
            ctx->ack_count, ctx->done_received);
}

static void client_finish(struct client_stream_ctx* st, const char* reason)
{
    if (!st->close_issued) {
        st->close_issued = 1;
...
static lsquic_conn_ctx_t* on_new_conn(void* stream_if_ctx, lsquic_conn_t* conn)
{
    ...
    conn_h->conn = conn;
    conn_h->app_ctx = ctx;
    if (!ctx->quiet) fprintf(stderr, "[client] new connection\n");
...
static void on_hsk_done(lsquic_conn_t* conn, enum lsquic_hsk_status status)
{
    ...
    if (!conn_h || !conn_h->app_ctx) return;
...
static void on_conn_closed(lsquic_conn_t* conn)
{
    ...
    if (ctx) {
        if (!ctx->final_printed) print_final_result(ctx, ctx->connect_failed ? "connect-error" : "loop-exit");
        prog_stop(ctx->prog);
    }
...
static lsquic_stream_ctx_t* on_new_stream(void* stream_if_ctx, lsquic_stream_t* stream)
{
    ...
    for (i = 0; i < st->chunk_bytes; ++i) st->chunk[i] = (unsigned char)(i & 0xFF);
    ...
    if (!ctx->quiet) fprintf(stderr, "[client] new stream\n");
    lsquic_stream_wantread(stream, 1);
    lsquic_stream_wantwrite(stream, 1);
    return (lsquic_stream_ctx_t*)st;
}

static void handle_server_line(struct client_stream_ctx* st, const char* line)
{
    ...
    if (0 == strncmp(line, "DONE bytes=", 11)) {
        ...
        ctx->done_received = 1;
        ctx->done_ms = now_ms();
        if (0 != finalize_local_hash(ctx)) ctx->connect_failed = 1;
        client_finish(st, (ctx->done_bytes == ctx->sent_bytes && 0 == strcmp(ctx->local_hash, ctx->server_hash))
                              ? "done"
                              : "mismatch");
        return;
    }
...
static int append_recv_buf(struct client_stream_ctx* st, const unsigned char* data, size_t len)
{
    char*  nbuf;
    size_t new_cap;
    ...
    while (new_cap < st->recv_buf_len + len) new_cap *= 2;
...
static void compact_recv_buf(struct client_stream_ctx* st)
{
    if (st->recv_parse_off == 0) return;
    ...
    memmove(st->recv_buf, st->recv_buf + st->recv_parse_off, st->recv_buf_len - st->recv_parse_off);
...
static void on_write(lsquic_stream_t* stream, lsquic_stream_ctx_t* st_h)
{
    ...
    while (st->header_off < st->upload_header_len) {
        nw = lsquic_stream_write(stream, st->upload_header + st->header_off, st->upload_header_len - st->header_off);
        if (nw > 0)
    ...
    while (st->sent_bytes < st->total_bytes) {
        ...
        if (nw > 0) {
            ...
            if (st->send_off == st->chunk_bytes) st->send_off = 0;
            if ((size_t)nw < tosend) break;
        } else if (nw == 0 || errno == EWOULDBLOCK)
            break;
        else {
            ctx->connect_failed = 1;
...
static void on_close(lsquic_stream_t* stream, lsquic_stream_ctx_t* st_h)
{
    ...
    if (st) {
        if (st->app_ctx) st->app_ctx->stream_ctx = NULL;
        free(st->chunk);
...
{
    fprintf(stderr,
            "Usage: %s [--server-ip IP] [--server-port PORT] [--total-bytes BYTES] [--chunk-bytes BYTES] [--count N] "
            "[--length BYTES] [--silent]\n",
            prog);
}
...
int main(int argc, char** argv)
{
    ...
    if (count) total_bytes = count * chunk_bytes;
    ...
    if (0 != prog_set_opt(&prog, 'H', "localhost")) return 1;

    if (0 != prog_prep(&prog)) return 1;
    if (0 != prog_connect(&prog, NULL, 0)) return 1;

    if (!quiet)
        fprintf(stderr, "[client] connecting to %s:%hu, total_bytes=%zu, chunk_bytes=%zu\n", server_ip, server_port,
                total_bytes, chunk_bytes);

    prog_run(&prog);
    prog_cleanup(&prog);
    if (client_ctx.xxh_state) XXH3_freeState(client_ctx.xxh_state);
