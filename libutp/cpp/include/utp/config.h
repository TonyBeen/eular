...
public:
    ...
    uint32_t           bbr_init_cwnd_mss = 16;                  ///< BBR 初始拥塞窗口 (MSS)
    ...
    uint8_t  handshake_max_retries = 2;                         ///< 握手超时后的最大重试次数
    uint32_t pending_incoming_limit = 1024;                     ///< 全局未完成被动握手上限
    uint32_t pending_incoming_per_ip_limit = 32;                ///< 单 IP 未完成被动握手上限
    uint32_t pending_unaccepted_timeout_ms = 3000;              ///< 等待应用 accept 的最长时间
    ...
    uint16_t init_max_streams_bidi = 32;                        ///< 初始最大双向流数
    uint16_t init_max_streams_uni = 16;                         ///< 初始最大单向流数
    uint64_t initial_max_data = 8ull * 1024ull * 1024ull;       ///< 初始连接级流量控制窗口
    uint64_t initial_max_stream_data_bidi_local = 256 * 1024;   ///< 对端可向本端双向流发送的初始窗口
    uint64_t initial_max_stream_data_bidi_remote = 256 * 1024;  ///< 本端可向对端双向流发送的初始窗口
    ...
    uint32_t            stream_drr_deficit_cap = 128 * 1024;              ///< DRR 赤字上限
    uint32_t            stream_send_buffer_limit = 256 * 1024;            ///< 单流写缓冲区上限
    ...
    // --- Receive Reassembly Limits ---
    uint32_t recv_reassembly_memory_limit = 16 * 1024 * 1024;       ///< 单连接乱序重组实际内存上限
    uint32_t recv_reassembly_fragment_limit = 4096;                  ///< 单连接乱序片段上限
    uint32_t recv_stream_reassembly_memory_limit = 4 * 1024 * 1024; ///< 单流乱序重组实际内存上限
    uint32_t recv_stream_reassembly_fragment_limit = 1024;           ///< 单流乱序片段上限
    uint32_t recv_stream_max_gap = 2 * 1024 * 1024;                  ///< 单帧相对当前读取点的最大前向间隙
    ...
    uint32_t                connection_wdrr_deficit_cap = 512 * 1024;              ///< 连接级 WDRR 赤字上限
};
