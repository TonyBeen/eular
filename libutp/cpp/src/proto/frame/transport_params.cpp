...
int32_t FrameTransportParams::decode(const void *buffer, size_t size, Status &status)
{
    ...
    if ((params->flags & ~TransportParams::kDefaultFlags) != 0) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "transport params contain unknown flags");
        return -1;
    }
    if (params->ack_delay_exponent > TransportParams::kMaxAckDelayExponent) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "ACK delay exponent exceeds protocol limit");
        return -1;
    }
    if (params->initial_max_data > TransportParams::kMaxFlowControlValue ||
        params->initial_max_stream_data_bidi_local > TransportParams::kMaxFlowControlValue ||
        params->initial_max_stream_data_bidi_remote > TransportParams::kMaxFlowControlValue) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "flow-control transport parameter exceeds protocol limit");
        return -1;
    }
