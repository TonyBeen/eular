...
{
    if (buffer == nullptr || _history == nullptr || _params == nullptr) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "invalid ack encode args");
        return -1;
    }
    if (_params->ack_delay_exponent > TransportParams::kMaxAckDelayExponent) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "ACK delay exponent exceeds protocol limit");
        return -1;
    }
    ...
    const utp_time_t largestReceivedUs = _history->largestAckedReceived();
    const utp_time_t delta = _now >= largestReceivedUs ? _now - largestReceivedUs : 0;
    const utp_time_t encodedDelay = delta >> _params->ack_delay_exponent;
    uint16_t ackDelay = static_cast<uint16_t>(
        std::min<utp_time_t>(encodedDelay, (std::numeric_limits<uint16_t>::max)()));
...
int32_t eular::utp::FrameAck::decode(const void *buffer, size_t size, Status &status)
{
    ...
    _ackInfo->ack_delay = static_cast<utp_time_t>(ackDelay) << _params->ack_delay_exponent;
