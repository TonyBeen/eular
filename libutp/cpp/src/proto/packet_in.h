...
struct PacketIn {
public:
    Status decode(const void *buffer, size_t size);
    ...
    int32_t nextFrame(size_t &offset, FrameType &frameType, const uint8_t *&frameData, size_t &frameLen,
                      Status &status) const;

private:
    static Status frameLength(const uint8_t *frameData, size_t payloadLeft, FrameType &frameType, size_t &frameLen);
