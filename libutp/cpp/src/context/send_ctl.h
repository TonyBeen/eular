...
public:
    ...
    void        notePacketSent(utp_packno_t packetNo);

public:
    bool        canSend();
    bool        canSchedulePacket(uint32_t packetSize) const;
    uint64_t    bytesOutTotal() const;
    uint64_t    bandwidthEstimate() const;
    uint64_t    retransmittedBytes() const;
    utp_packno_t largestSentPacketNo() const { return m_sendHistory.largestPackNo(); }
...
private:
    ...
    bool        canTransmitPacket(uint32_t packetSize);
    void        notePacketPaced(uint32_t packetSize);
