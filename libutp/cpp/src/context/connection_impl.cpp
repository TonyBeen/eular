...
size_t EncryptionKeySize(eular::utp::FrameCryptoType type)
{
    return type == eular::utp::kFrameCryptoAESGCM256 ? 32u : 16u;
}
...
void ConnectionImpl::bootstrapLocalTransportParams()
{
    ...
    m_loaclTP.ack_delay_exponent =
        std::min<uint8_t>(cfg->ack_delay_exponent, TransportParams::kMaxAckDelayExponent);
    ...
    m_localBytesReceivedTotal = 0;
    m_localStreamMaxReceivedOffset.clear();
...
Status ConnectionImpl::initPassive(const Context::ConnectInfo& info, const Address& peerAddress,
                                   ...
                                   const std::shared_ptr<AesGcmContext>& txAesCtx,
                                   const std::shared_ptr<AesGcmContext>& rxAesCtx)
{
    if (m_state != State::kStateDisconnected) {
        return Status::ErrorLiteral(UTP_ERR_INVALID_STATE, "connection is not in disconnected state");
    }
...
void ConnectionImpl::onUdpPacket(const UdpSocket::MsgMetaInfo& msg, utp_time_t nowUs)
{
    ...
    if (!detail::DecodeUdpPacketWithOptionalAead(msg, m_mm, m_rxAesCtx, *packet)) {
        return;
    }
    ...
    if (packetPn == 0 || m_receiveHistory.contains(packetPn)) {
        return;
    }
        ...
        switch (frameType) {
            ...
            case kFrameAck: {
                ...
                if (ack.decode(frameData, frameLen, st) >= 0 && m_sendCtl) {
                    const Status ackStatus = m_sendCtl->onAckReceived(ackInfo, nowUs);
                    if (!ackStatus.ok()) {
                        (void)recordConnectionError(ackStatus, true);
                        abortConnection(ackStatus.code(), ackStatus.code(), ackStatus.message());
                        fatalPacketError = true;
                        stopFrameProcessing = true;
                    } else {
                        scheduleWrite();
            ...
            case kFrameTransportParams: {
                ...
                Status st;
                if (packet->header.types != UTP_TYPE_INITIAL && packet->header.types != UTP_TYPE_HANDSHAKE) {
                    st = Status::ErrorLiteral(UTP_ERR_FRAME_UNEXPECTED,
                                              "transport params are only valid during handshake");
                } else if (frameTp.decode(frameData, frameLen, st) >= 0) {
                    m_peerTP = peerTp;
                ...
                if (!st.ok()) {
                    (void)recordConnectionError(st, true);
                    abortConnection(st.code(), st.code(), st.message());
                    fatalPacketError = true;
                    stopFrameProcessing = true;
                }
                break;
            ...
            case kFrameCrypto: {
                ...
                Status st;
                if (packet->header.types != UTP_TYPE_INITIAL && packet->header.types != UTP_TYPE_HANDSHAKE) {
                    st = Status::ErrorLiteral(UTP_ERR_FRAME_UNEXPECTED, "crypto frame is only valid during handshake");
                } else if (crypto.decode(frameData, frameLen, st) >= 0) {
                    if (!m_x25519) {
                    ...
                    try {
                        ...
                        const Status keyStatus = TrafficKeySchedule::CreateAesGcmContexts(
                            *m_x25519, peerPublicKey, m_localConnectionID, m_peerConnectionID, true,
                            static_cast<uint8_t>(crypto.crypto_type), EncryptionKeySize(crypto.crypto_type),
                            m_txAesCtx, m_rxAesCtx);
                        if (!keyStatus.ok()) {
    ...
    // Handshake packets are not ACK-eliciting here, but recording their packet
    // numbers prevents replay and keeps one packet-number space per connection.
    if (!streamBackpressured && !closePacket) {
    ...
    if (m_state == State::kStateInitialSent && packet->header.types == UTP_TYPE_HANDSHAKE) {
        if (m_connectInfo.encrypted != Context::kEncryptionNone && (!m_txAesCtx || !m_rxAesCtx)) {
            const Status cryptoStatus = Status::ErrorLiteral(
                UTP_ERR_CRYPTO_INIT_FAILED, "encrypted handshake completed without traffic keys");
            (void)recordConnectionError(cryptoStatus, true);
            abortConnection(cryptoStatus.code(), cryptoStatus.code(), cryptoStatus.message());
            return;
        }
        m_state = State::kStateConnected;
        m_handshakeRetryCount = 0;
...
void ConnectionImpl::adoptNextPacketNumber(uint64_t nextPacketNumber)
{
    if (nextPacketNumber <= m_packetNumber) {
        return;
    }
    m_packetNumber = nextPacketNumber;
    if (m_sendCtl != nullptr) {
        m_sendCtl->notePacketSent(nextPacketNumber - 1);
    }
}
...
Status ConnectionImpl::ingestStreamFrame(const FrameStream& streamFrame, PacketIn* packet)
{
    ...
    if (streamFrame.stream_offset > (std::numeric_limits<uint64_t>::max)() - streamFrame.stream_data_length) {
    ...
    const uint64_t frameEnd = streamFrame.stream_offset + streamFrame.stream_data_length;
    auto advertisedIt = m_localMaxStreamDataAdvertised.find(streamFrame.stream_id);
    const uint64_t streamLimit = advertisedIt != m_localMaxStreamDataAdvertised.end()
                                     ? advertisedIt->second
                                     : initialStreamReceiveWindow(streamFrame.stream_id);
    if (frameEnd > streamLimit) {
        return Status::ErrorLiteral(UTP_ERR_STREAM_FLOW_CONTROL, "peer exceeded stream receive window");
    }

    auto receivedIt = m_localStreamMaxReceivedOffset.find(streamFrame.stream_id);
    const uint64_t oldStreamEnd = receivedIt != m_localStreamMaxReceivedOffset.end() ? receivedIt->second : 0;
    const uint64_t streamDelta = frameEnd > oldStreamEnd ? frameEnd - oldStreamEnd : 0;
    if (streamDelta > (std::numeric_limits<uint64_t>::max)() - m_localBytesReceivedTotal ||
        m_localBytesReceivedTotal + streamDelta > m_localMaxDataAdvertised) {
        return Status::ErrorLiteral(UTP_ERR_STREAM_FLOW_CONTROL, "peer exceeded connection receive window");
    }

    StreamImpl::SP stream;
    if (it == m_streams.end()) {
        stream = std::make_shared<StreamImpl>(this, streamFrame.stream_id, defaultStreamPriority());
    ...
    const Status status = stream->onFrame(streamFrame, packet);
    if (status.ok() && streamDelta > 0) {
        m_localStreamMaxReceivedOffset[streamFrame.stream_id] = frameEnd;
        m_localBytesReceivedTotal += streamDelta;
    }
                                       ...
                                       bool fin)
{
    ...
    if (m_ackElicitingSinceLastAck > 0) {
        if (buildAckPayload(m_ackPayloadScratch, nowUs).ok()) {
            ...
            if (m_txAesCtx != nullptr && packetPayloadBudget > AesGcmContext::GCM_TAG_SIZE) {
                packetPayloadBudget -= AesGcmContext::GCM_TAG_SIZE;
            }
...
void ConnectionImpl::ensureFlowControlAdvertised(uint32_t streamId)
{
    ...
    if (it == m_localMaxStreamDataAdvertised.end()) {
        const uint64_t initialStreamWindow = initialStreamReceiveWindow(streamId);
        m_localMaxStreamDataAdvertised.emplace(streamId, initialStreamWindow);
    }
}

uint64_t ConnectionImpl::initialStreamReceiveWindow(uint32_t streamId) const
{
    const bool locallyInitiated = m_isClientInitiator ? STREAM_ID_IS_CLIENT(streamId) : STREAM_ID_IS_SERVER(streamId);
    const uint64_t configured = locallyInitiated ? m_loaclTP.initial_max_stream_data_bidi_local
                                                 : m_loaclTP.initial_max_stream_data_bidi_remote;
    return configured > 0 ? configured : kDefaultInitialMaxStreamData;
}
...
void ConnectionImpl::onStreamBytesConsumed(uint32_t streamId, size_t bytes)
{
    ...
    const uint64_t baseMaxStreamData = initialStreamReceiveWindow(streamId);
                                  ...
                                  size_t frameMetaCount)
{
    ...
    const bool   shouldEncrypt = (m_txAesCtx != nullptr) && ((packetFlags & PacketOutFlags::kPoNoEncrypt) == 0) &&
    ...
    const bool   shouldSchedule = ((packetFlags & PacketOutFlags::kPoSched) != 0) ||
                                  (shouldTrackPacket && !isClosePacket && !isMtuProbePacket &&
    ...
    if (shouldSchedule && (!m_sendCtl || !m_sendCtl->canSchedulePacket(static_cast<uint32_t>(wirePacketLen)))) {
        return Status::ErrorLiteral(UTP_ERR_WOULD_BLOCK, "congestion window blocked");
    }
    ...
    packet->po_flags |= static_cast<uint16_t>(packetFlags & ~PacketOutFlags::kPoSched);
    ...
    if (!useScatterEncryptOnly) {
        ...
        Status encSt =
            m_txAesCtx->encryptScatter(plainSegments.data(), plainCount, packet->raw_data, UTP_HEADER_SIZE,
    ...
    if (shouldEncrypt && !useScatterEncryptOnly) {
        Status encSt = m_txAesCtx->encrypt(packet);
        if (!encSt.ok()) {
            m_mm.putPacketOut(packet);
            return encSt;
        }
    ...
    if (m_sendCtl != nullptr) {
        m_sendCtl->notePacketSent(packet->packno);
    }
