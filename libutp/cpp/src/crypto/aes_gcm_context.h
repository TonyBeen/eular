...
public:
    ...
    using NoncePrefix = std::array<uint8_t, 4>;
    ...
    Status init(const AesKey128& key, const NoncePrefix& noncePrefix);
    ...
    Status init(const AesKey256& key, const NoncePrefix& noncePrefix);
    ...
    static void     PurgeThreadLocalEncryptBuffers();
    static size_t   CachedThreadLocalEncryptBufferCount();
...
private:
    ...
    size_t                  m_keySize{0};
    NoncePrefix             m_noncePerfix; // 32-bit prefix + 64-bit packet number
};
