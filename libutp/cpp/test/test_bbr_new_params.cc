...
TEST_CASE("BbrV1: new configurable params are applied", "[BBR]")
{
    ...
    REQUIRE(bbr.m_recoveryState == BbrV1::RecoveryState::NotInRecovery);
    REQUIRE(bbr.m_currentRoundTripEnd == 0);
    REQUIRE(bbr.m_endRecoveryAt == 0);
    REQUIRE(bbr.m_recoveryWindow == 0);
    REQUIRE(bbr.m_minRttSinceLastProbe == UINT64_MAX);
