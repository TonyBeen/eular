...
bool HasOutstandingPacketWithBits(const eular::utp::SendControl *sendCtl, uint32_t bits)
{
    if (HasUnackedPacketWithBits(sendCtl, bits)) {
        return true;
    }
    if (sendCtl == nullptr) {
        return false;
                              ...
                              uint16_t payloadBytes = 1)
{
    ...
    pkt->stream_data_size = streamDataBytes;
    pkt->transient_ack_size = transientAckBytes;
    conn.m_sendCtl->notePacketSent(packno);
...
TEST_CASE("Ack piggyback coalesces pending ACK with outgoing stream packet", "[Ack][Integration]")
{
    ...
    REQUIRE(PumpUntil(
        ...
        200,
        1));
    eular::utp::PacketOut *ackStreamPacket =
        FindUnackedPacketWithBits(clientConn->m_sendCtl.get(), ackStreamBits);
    REQUIRE(ackStreamPacket != nullptr);
    REQUIRE(ackStreamPacket->transient_ack_size > 0);
    REQUIRE(ackStreamPacket->frame_meta_count >= 2);
    REQUIRE(ackStreamPacket->frame_meta[0].frame_type == FrameType::kFrameAck);
}
...
TEST_CASE("Adaptive AckFrequency updates on sustained RTT increase", "[Ack][Integration]")
{
    ...
    clientConn->m_rttStats = eular::utp::RttStats();
    clientConn->m_rttStats.update(200000);
    ...
    REQUIRE(HasOutstandingPacketWithBits(clientConn->m_sendCtl.get(), ackFrequencyBits));
}
...
TEST_CASE("ACK claiming an unsent packet is rejected without releasing inflight data", "[Ack][Protocol][Regression]")
{
    Config cfg;
    ev::EventLoop loop;
    ContextImpl ctx(loop.loop(), &cfg);
    ConnectionImpl conn(&ctx, nullptr, 2110);

    BuildUnackedPacket(conn, 10, (1u << static_cast<uint32_t>(FrameType::kFrameStream)), 16);
    const Status st = conn.m_sendCtl->onAckReceived(BuildAckRange(11, 11), eular::utp::time::MonotonicUs());

    REQUIRE(st.code() == UTP_ERR_FRAME_FORMAT_ERROR);
    REQUIRE_FALSE(TAILQ_EMPTY(&conn.m_sendCtl->m_unackedPackets));
    REQUIRE(conn.m_sendCtl->m_largestAckedPackNo == 0);
}

TEST_CASE("ACK delay used for RTT sampling is capped by peer maximum", "[Ack][RTT][Regression]")
{
    Config cfg;
    ev::EventLoop loop;
    ContextImpl ctx(loop.loop(), &cfg);
    ConnectionImpl conn(&ctx, nullptr, 2111);
    conn.m_peerAckMaxDelayMs = 10;

    PacketOut *pkt = BuildUnackedPacket(
        conn, 20, (1u << static_cast<uint32_t>(FrameType::kFrameStream)), 16);
    const utp_time_t nowUs = eular::utp::time::MonotonicUs();
    pkt->sent_time = nowUs - 100000;

    const Status st = conn.m_sendCtl->onAckReceived(BuildAckRange(20, 20, 50000), nowUs);
    REQUIRE(st.ok());
    REQUIRE(conn.m_obsRttUs == 90000);
}
