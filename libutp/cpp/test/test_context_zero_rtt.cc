...
TEST_CASE("ContextImpl: encrypted 0-RTT tickets are not issued", "[Context][0RTT][Security]")
{
    Config cfg;
    ev::EventLoop loop;
    ContextImpl ctx(loop.loop(), &cfg);
    ...
    REQUIRE_FALSE(ctx.buildZeroRttSessionToken(Address("127.0.0.1", 9000), 1234,
                                               Context::kEncryptionAesGcm256, validity, ticket));
    REQUIRE(validity == 0);
