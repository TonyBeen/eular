...
TEST_CASE("AES encrypt buffer cache is bounded and purgeable", "[MemoryManager][Crypto][Regression]")
{
    using eular::utp::AesGcmContext;
    AesGcmContext::PurgeThreadLocalEncryptBuffers();

    std::vector<uint8_t*> buffers;
    for (size_t i = 0; i < 100; ++i) {
        uint8_t* buffer = AesGcmContext::AcquireEncryptBuffer(1400);
        REQUIRE(buffer != nullptr);
        buffers.push_back(buffer);
    }
    for (uint8_t* buffer : buffers) {
        AesGcmContext::ReleaseEncryptBuffer(buffer, 1400);
    }

    REQUIRE(AesGcmContext::CachedThreadLocalEncryptBufferCount() == 8);
    AesGcmContext::PurgeThreadLocalEncryptBuffers();
    REQUIRE(AesGcmContext::CachedThreadLocalEncryptBufferCount() == 0);
}
