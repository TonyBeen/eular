...
TEST_CASE("ReceiveHistory: contains covers retained and discarded packet numbers", "[ReceiveHistory][Replay]")
{
    ReceiveHistory hist(/*maxRanges=*/2);
    hist.insert(10, 100);
    hist.insert(12, 120);

    REQUIRE(hist.contains(10));
    REQUIRE_FALSE(hist.contains(11));
    REQUIRE(hist.contains(12));

    hist.insert(14, 140);
    REQUIRE(hist.cutoff() == 11);
    REQUIRE(hist.contains(10));
    REQUIRE_FALSE(hist.contains(13));
    REQUIRE(hist.contains(14));
}
