...
> 状态：未来设计草案，当前未落地。本文不描述现行报文格式或 API。
> 当前 C11 加密和 0-RTT 行为请参阅
> [加密与 0-RTT 实现细节](加密与0-RTT实现细节.md)，并以 `utp/include/utp/`、`utp/src/crypto/` 为准。
> 文中的 `cpp/` 和 `src/...` 路径仅是历史实现/方案落点示例，不能作为当前代码路径。
> 当前讨论的低改动 wire 方案见
> [全包轻量混淆与 Padding 方案](../proto/全包轻量混淆与Padding方案.md)。两者是互斥的候选 wire profile，不应叠加解读。
