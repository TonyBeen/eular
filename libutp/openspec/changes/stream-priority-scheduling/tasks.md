...
- [x] 2.1 在连接发送路径引入 stream 优先级调度器。
- [x] 2.2 实现 Strict Priority + Aging 与 DRR 两种调度策略（默认 Strict）。
- [x] 2.3 增加配置项（默认优先级、开关、Aging 参数、DRR 参数、三态热切换）。
- [ ] 2.4 打通指标采集（排队时延、公平性、DRR deficit、策略切换）。

## 3. Validation

- [x] 3.1 新增单测：不同优先级流的调度顺序与饥饿保护。
- [x] 3.2 新增单测：流控约束选流、DRR 调度公平性、策略热切换。
