...
int32_t AesGcmContext::encrypt(PacketOut *packet)
{
    ...
    uint8_t *encrypted = static_cast<uint8_t *>(std::malloc(encryptedPacketLen));
    ...
    std::memcpy(encrypted, packet->raw_data, UTP_HEADER_SIZE);
    StoreBE16(encrypted + kPayloadLengthOffset, static_cast<uint16_t>(cipherPayloadLen));
