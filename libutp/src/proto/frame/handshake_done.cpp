...
int32_t FrameHandshakeDone::encode(void *buffer, size_t size, Status &status) const
{
    if (size < FRAME_HANDSHAKE_DONE_SIZE) {
        status = Status::Error(UTP_ERR_OVERFLOW,
                               fmt::format("buffer size {} is smaller than handshake done frame size {}",
                                           size,
    ...
    if (offset == nullptr) {
        status = Status::ErrorLiteral(UTP_ERR_OVERFLOW, "encode handshake done frame failed");
        return -1;
    }
...
int32_t FrameHandshakeDone::decode(const void *buffer, size_t size, Status &status)
{
    if (size < FRAME_HANDSHAKE_DONE_SIZE) {
        status = Status::Error(UTP_ERR_OVERFLOW,
                               fmt::format("buffer size {} is smaller than handshake done frame size {}",
    ...
    if (offset == nullptr) {
        status = Status::ErrorLiteral(UTP_ERR_OVERFLOW, "decode handshake done frame failed");
        return -1;
    }
