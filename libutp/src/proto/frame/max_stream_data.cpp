...
int32_t FrameMaxStreamData::encode(void *buffer, size_t size, Status &status) const
{
    if (size < FRAME_MAX_STREAM_DATA_SIZE) {
        status = Status::Error(UTP_ERR_OVERFLOW,
                               fmt::format("buffer size {} is smaller than max stream data frame size {}",
                                           size,
    ...
    if (offset == nullptr) {
        status = Status::ErrorLiteral(UTP_ERR_OVERFLOW, "encode max stream data frame failed");
        return -1;
    }
...
int32_t FrameMaxStreamData::decode(const void *buffer, size_t size, Status &status)
{
    if (size < FRAME_MAX_STREAM_DATA_SIZE) {
        status = Status::Error(UTP_ERR_OVERFLOW,
                               fmt::format("buffer size {} is smaller than max stream data frame size {}",
    ...
    if (offset == nullptr) {
        status = Status::ErrorLiteral(UTP_ERR_OVERFLOW, "decode max stream data frame failed");
        return -1;
    }
