...
{
    // A queued callback may carry a timestamp captured before another write
    // callback advanced the pacer. Never abort or move the pacing clock back.
    _now = std::max(now, _now);
