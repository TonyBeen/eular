...
private:
    ...
    struct PendingIncomingConnection {
        ...
        std::shared_ptr<AesGcmContext> txAesCtx;
        std::shared_ptr<AesGcmContext> rxAesCtx;
    };
...
private:
    ...
    bool    decodeIncomingPendingPacket(const UdpSocket::MsgMetaInfo &msg,
                                        ...
                                        PacketIn &packet);
    Status  parsePendingNegotiationFrame(PendingIncomingConnection &pending,
                                         uint8_t frameType,
    ...
    bool    canCreatePendingIncoming(const std::string &peerIp) const;
    void    notePendingIncomingAdded(const std::string &peerIp);
    void    notePendingIncomingRemoved(const std::string &peerIp);
...
private:
    ...
    std::unordered_map<std::string, uint32_t> m_pendingIncomingByPeerIp;
