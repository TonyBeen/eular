                                          ...
                                          const std::string &state)
{
    ...
    if (parsed.encrypted != Context::kEncryptionNone) {
        return Status::ErrorLiteral(UTP_ERR_NOT_IMPLEMENTED,
                                    "encrypted 0-RTT is disabled until an authenticated early-data key format exists");
    }
                                           ...
                                           std::vector<uint8_t> &outToken)
{
    ...
    if (!peerAddress.isValid() || cid == 0 || encrypted != Context::kEncryptionNone) {
        return false;
    }
...
Status  ContextImpl::sendPendingHandshake(PendingIncomingConnection &pending)
{
    ...
    localTp.ack_delay_exponent =
        std::min<uint8_t>(m_config.ack_delay_exponent, TransportParams::kMaxAckDelayExponent);
...
bool ContextImpl::decodeIncomingPendingPacket(const UdpSocket::MsgMetaInfo &msg,
                                              ...
                                              PacketIn &packet)
{
    return detail::DecodeUdpPacketWithOptionalAead(msg, m_mm, pending.rxAesCtx, packet);
}

Status ContextImpl::parsePendingNegotiationFrame(PendingIncomingConnection &pending,
                                                 uint8_t frameType,
                                                                 ...
                                                                 uint64_t nextPacketNumber)
{
    ...
    if (conn->initPassive(info, peerAddress, peerCid, peerTp, peerAckFrequency, x25519, txAesCtx, rxAesCtx) !=
        UTP_ERR_OK) {
        return ConnectionImpl::SP();
    }
    conn->adoptNextPacketNumber(nextPacketNumber);
...
void ContextImpl::removePendingIncoming(uint32_t localCid)
{
    ...
    notePendingIncomingRemoved(it->second.peerIp);
    m_pendingIncoming.erase(it);
}

bool ContextImpl::canCreatePendingIncoming(const std::string &peerIp) const
{
    const uint32_t globalLimit = std::max<uint32_t>(m_config.pending_incoming_limit, 1);
    if (m_pendingIncoming.size() >= globalLimit) {
        return false;
    }

    const uint32_t perIpLimit = std::max<uint32_t>(m_config.pending_incoming_per_ip_limit, 1);
    auto it = m_pendingIncomingByPeerIp.find(peerIp);
    return it == m_pendingIncomingByPeerIp.end() || it->second < perIpLimit;
}

void ContextImpl::notePendingIncomingAdded(const std::string &peerIp)
{
    if (!peerIp.empty()) {
        ++m_pendingIncomingByPeerIp[peerIp];
    }
}

void ContextImpl::notePendingIncomingRemoved(const std::string &peerIp)
{
    auto it = m_pendingIncomingByPeerIp.find(peerIp);
    if (it == m_pendingIncomingByPeerIp.end()) {
        return;
    }
    if (it->second <= 1) {
        m_pendingIncomingByPeerIp.erase(it);
    } else {
        --it->second;
    }
}
...
{
    if (m_pendingIncoming.empty()) {
        return;
    }

    const utp_time_t nowUs = time::MonotonicUs();
    std::vector<uint32_t> expired;
    expired.reserve(m_pendingIncoming.size());

    for (auto &entry : m_pendingIncoming) {
        const uint32_t localCid = entry.first;
        PendingIncomingConnection &pending = entry.second;
        if (!pending.handshakeSent) {
            const uint64_t timeoutUs = static_cast<uint64_t>(
                                           std::max<uint32_t>(m_config.pending_unaccepted_timeout_ms, 1)) *
                                       1000ULL;
            if (pending.initialReceivedUs == 0 ||
                static_cast<uint64_t>(nowUs) >= static_cast<uint64_t>(pending.initialReceivedUs) + timeoutUs) {
                expired.push_back(localCid);
            }
            continue;
        }

        if (pending.lastHandshakeSentUs == 0) {
            expired.push_back(localCid);
            continue;
        }
...
void ContextImpl::refreshPendingHandshakeTimer()
{
    ...
    for (const auto &entry : m_pendingIncoming) {
        const PendingIncomingConnection &pending = entry.second;
        if (!pending.handshakeSent) {
            const uint64_t timeoutUs = static_cast<uint64_t>(
                                           std::max<uint32_t>(m_config.pending_unaccepted_timeout_ms, 1)) *
                                       1000ULL;
            const uint64_t dueUs = static_cast<uint64_t>(pending.initialReceivedUs) + timeoutUs;
            nextDueUs = std::min(nextDueUs, dueUs);
            continue;
        }
    ...
    while (true) {
        ...
        for (int32_t i = 0; i < nread; ++i) {
            ...
            if (pendingIt != m_pendingIncoming.end() && pendingIt->second.handshakeSent) {
                ...
                if (decodeOk) {
                    ...
                    while (frameOffset < pendingPacket->payload_size) {
                        ...
                        const Status negotiationStatus = parsePendingNegotiationFrame(
                            pendingIt->second, static_cast<uint8_t>(frameType), frameData, frameLen);
                        if (!negotiationStatus.ok()) {
                            decodeOk = false;
                            break;
                        }
            ...
            if (packetType == UTP_TYPE_0RTT) {
                ...
                if (ticketEncryptionMode != Context::kEncryptionNone) {
                    PendingIncomingConnection decision;
                    decision.peerAddress = msg.metaInfo.peerAddress;
                    decision.peerIp = msg.metaInfo.peerAddress.toIpString();
                    decision.peerCid = scid;
                    ++m_stat.zero_rtt_rejected;
                    reportZeroRttDecision(decision, false, "encrypted_0rtt_disabled");
                    (void)sendPendingConnectionClose(decision, UTP_ERR_NOT_IMPLEMENTED,
                                                     "encrypted_0rtt_disabled");
                    continue;
                }
                ...
                conn->adoptNextPacketNumber(sendCtx.packetNumber);
            ...
            if (!canCreatePendingIncoming(peerIp)) {
                continue;
            }
            ...
            while (frameOffset < initialPacket.payload_size) {
                ...
                if (initialPacket.nextFrame(frameOffset, frameType, frameData, frameLen, nextSt) < 0) {
                    negotiationValid = false;
                    break;
                }

                const Status negotiationStatus = parsePendingNegotiationFrame(
                    pending, static_cast<uint8_t>(frameType), frameData, frameLen);
                if (!negotiationStatus.ok()) {
                    negotiationValid = false;
                    break;
                }

                if (frameType == kFrameCrypto && pending.rxAesCtx == nullptr) {
                    ...
                    if (st.ok()) {
                        ...
                        try {
                            ...
                            const Status keyStatus = TrafficKeySchedule::CreateAesGcmContexts(
                                *pending.x25519, peerPublicKey, pending.localCid, pending.peerCid, false,
                                static_cast<uint8_t>(crypto.crypto_type), EncryptionKeySize(crypto.crypto_type),
                                pending.txAesCtx, pending.rxAesCtx);
                            if (!keyStatus.ok()) {
                                pending.txAesCtx.reset();
                                pending.rxAesCtx.reset();
                                negotiationValid = false;
                                break;
                            }
                        } catch (const std::exception &) {
            ...
            if (!negotiationValid) {
                (void)sendPendingConnectionClose(pending, UTP_ERR_INVALID_PARAM, "invalid handshake negotiation");
                continue;
            }
            ...
            notePendingIncomingAdded(peerIp);
            refreshPendingHandshakeTimer();
