...
public:
    ...
    void                               adoptNextPacketNumber(uint64_t nextPacketNumber);
...
private:
    ...
    uint64_t initialStreamReceiveWindow(uint32_t streamId) const;
...
private:
    ...
    uint64_t                               m_localBytesReceivedTotal{0};
    std::unordered_map<uint32_t, uint64_t> m_localStreamMaxReceivedOffset;
    ...
    std::shared_ptr<AesGcmContext> m_txAesCtx;
    std::shared_ptr<AesGcmContext> m_rxAesCtx;
    ...
    size_t                              m_recvReassemblyMemoryBytes{0};
    size_t                              m_recvReassemblyFragmentCount{0};
