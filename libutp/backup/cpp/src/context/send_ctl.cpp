...
bool IsAckInfoValid(const eular::utp::AckInfo& ackInfo, utp_packno_t largestSent)
{
    if (ackInfo.range_size == 0 || ackInfo.range_size > ackInfo.ack_ranges.size() ||
        ackInfo.largest_ack_packno == 0 || ackInfo.largest_ack_packno > largestSent) {
        return false;
    }

    utp_packno_t observedLargest = 0;
    for (uint32_t i = 0; i < ackInfo.range_size; ++i) {
        const Range& range = ackInfo.ack_ranges[i];
        if (range.low == 0 || range.low > range.high || range.high > largestSent) {
            return false;
        }
        observedLargest = std::max(observedLargest, range.high);
        for (uint32_t j = 0; j < i; ++j) {
            const Range& previous = ackInfo.ack_ranges[j];
            if (range.low <= previous.high && previous.low <= range.high) {
                return false;
            }
        }
    }
    return observedLargest == ackInfo.largest_ack_packno;
}
...
void SendControl::notePacketSent(utp_packno_t packetNo)
{
    if (packetNo != 0) {
        m_sendHistory.update(packetNo);
    }
}

bool SendControl::canSend()
{
    return canTransmitPacket(1);
}

bool SendControl::canSchedulePacket(uint32_t packetSize) const
{
    if (packetSize == 0 || !m_congestion) {
        return false;
    }

    const uint64_t used = m_bytesUnackedAll + m_bytesScheduled;
    const uint64_t cwnd = m_congestion->getCwnd();
    if (used == 0) {
        return true;
    }
    return used < cwnd && static_cast<uint64_t>(packetSize) <= (cwnd - used);
}

bool SendControl::canTransmitPacket(uint32_t packetSize)
{
    if (packetSize == 0 || !m_congestion) {
        return false;
    }
    ...
    const uint64_t cwnd = m_congestion->getCwnd();
    if (m_bytesUnackedAll != 0 &&
        (m_bytesUnackedAll >= cwnd || static_cast<uint64_t>(packetSize) > (cwnd - m_bytesUnackedAll))) {
        return false;
    }

    if (m_flags & SendCtlFlags::Pace) {
        if (m_pacer.canSchedule(m_nInflightAll)) {
            return true;
        }

        if (m_conn != nullptr) {
            const utp_time_t nowUs = time::MonotonicUs();
            const utp_time_t nextUs = m_pacer.nextSched();
            const utp_time_t delayMs = nextUs > nowUs ? std::max<utp_time_t>(1, (nextUs - nowUs + 999) / 1000) : 1;
...
void SendControl::notePacketPaced(uint32_t packetSize)
{
    if ((m_flags & SendCtlFlags::Pace) == 0 || !m_congestion || packetSize == 0) {
        return;
    }

    m_pacer.packetScheduled(m_nInflightAll, false, [this, packetSize] () -> utp_time_t {
        const uint64_t rate = m_congestion->getPacingRate(0);
        if (rate == 0) {
            return 1;
        }
        const uint64_t numerator = static_cast<uint64_t>(packetSize) * 1000000ULL;
        return std::max<utp_time_t>(1, (numerator + rate - 1) / rate);
    });
}
...
{
    if (!IsAckInfoValid(ackInfo, m_sendHistory.largestPackNo())) {
        return Status::ErrorLiteral(UTP_ERR_FRAME_FORMAT_ERROR, "ACK covers an invalid or unsent packet range");
    }
    ...
    if (largestAckedThisRound > 0) {
        ...
        if (m_conn != nullptr && nowUs > largestAckedSentTimeThisRound) {
            ...
            uint64_t ackDelayUs = static_cast<uint64_t>(ackInfo.ack_delay);
            if (m_conn != nullptr) {
                const uint64_t peerMaxAckDelayUs = static_cast<uint64_t>(m_conn->m_peerAckMaxDelayMs) * 1000ULL;
                ackDelayUs = std::min<uint64_t>(ackDelayUs, peerMaxAckDelayUs);
            }
...
void SendControl::onCanWrite(utp_time_t nowUs)
{
    ...
    m_pacer.tickIn(nowUs);
    struct PacerTickGuard {
        Pacer &pacer;
        utp_time_t now;
        ~PacerTickGuard() { pacer.tickOut(now); }
    } tickGuard{m_pacer, nowUs};
    ...
    if (!TAILQ_EMPTY(&m_scheduledPackets)) {
        PacketOut *next = TAILQ_FIRST(&m_scheduledPackets);
        if (next != nullptr && canTransmitPacket(PacketSentSize(next)) && m_conn != nullptr) {
...
int32_t SendControl::flushScheduledPackets(utp_time_t nowUs, uint32_t maxPackets, bool &sentAny)
{
    ...
    const uint32_t batchCap = std::min<uint32_t>(maxPackets, 1u);
    ...
    TAILQ_FOREACH(pkt, &m_scheduledPackets, po_next) {
        ...
        if (!canTransmitPacket(PacketSentSize(pkt))) {
            break;
        }
    ...
    for (int32_t i = 0; i < sent; ++i) {
        ...
        if ((sentPkt->local_flags & PacketOutLocalFlags::kPOLTrackOnSend) != 0) {
            ...
            notePacketPaced(PacketSentSize(sentPkt));
...
int32_t SendControl::retransmitLostBatch(utp_time_t nowUs, uint32_t maxPackets, bool &sentAny)
{
    ...
    TAILQ_FOREACH(pkt, &m_lostPackets, po_next) {
        ...
        if (needRegenerateCipher) {
            if (m_conn->m_txAesCtx->encrypt(pkt) != UTP_ERR_OK) {
                break;
            }
...
Status SendControl::retransmitLostPacket(PacketOut *pkt, utp_time_t nowUs)
{
    ...
    appendUnacked(pkt);
    notePacketPaced(PacketSentSize(pkt));
    if (pkt->frame_types & m_retxFrames) {
        retransAlarm(nowUs);
    }
