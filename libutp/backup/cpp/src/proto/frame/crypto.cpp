...
int32_t FrameCrypto::encode(void *buffer, size_t size, Status &status) const
{
    ...
    if (crypto_type != kFrameCryptoAESGCM128 && crypto_type != kFrameCryptoAESGCM256) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "unsupported crypto frame algorithm");
        return -1;
    }
...
int32_t FrameCrypto::decode(const void *buffer, size_t size, Status &status)
{
    ...
    if ((crypto_type != kFrameCryptoAESGCM128 && crypto_type != kFrameCryptoAESGCM256) || reserved != 0) {
        status = Status::ErrorLiteral(UTP_ERR_INVALID_PARAM, "invalid crypto frame algorithm or reserved bits");
        return -1;
    }
