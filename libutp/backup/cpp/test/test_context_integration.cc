...
void AppendBufferedPendingPacket(ContextImpl::PendingIncomingConnection &pending,
                                 const std::vector<uint8_t> &packetBytes)
{
    ContextImpl::PendingIncomingConnection::BufferedPendingPacket buffered;
    buffered.offset = static_cast<uint32_t>(pending.bufferedBeforeHandshakeDoneStorage.size());
    buffered.len = static_cast<uint32_t>(packetBytes.size());
    pending.bufferedBeforeHandshakeDoneStorage.insert(pending.bufferedBeforeHandshakeDoneStorage.end(),
                                                       packetBytes.begin(), packetBytes.end());
    pending.bufferedBeforeHandshakeDone.emplace_back(buffered);
    pending.bufferedBeforeHandshakeDoneBytes += packetBytes.size();
}
...
TEST_CASE("Context integration: plaintext 0-RTT accepted early data establishes directly", "[Context][Integration][0RTT]")
{
    Config cfg;
    ...
    ConnectionImpl::SP serverConn = FindConnectedByRemote(server, BoundPort(client));
    REQUIRE(serverConn != nullptr);
    REQUIRE(observedEncryption == Context::kEncryptionNone);
    REQUIRE(serverConn->connectInfo().encrypted == Context::kEncryptionNone);
...
TEST_CASE("Context integration: legacy encrypted 0-RTT ticket is rejected before early data delivery",
          "[Context][Integration][0RTT][Security]")
{
    Config cfg;
    ...
    server.setOnZeroRttDecision([&](const Context::ZeroRttDecisionInfo &info) {
        rejected = !info.accepted && info.reason == "encrypted_0rtt_disabled";
    });
    ...
    info.port = BoundPort(server);
    info.timeout = 100;
    info.session_ticket = BuildZeroRttTicket(server, Address("127.0.0.1", BoundPort(client)), 12346,
                                             Context::kEncryptionAesGcm256);
    info.early_data = {'s', 'e', 'c', 'r', 'e', 't'};
    REQUIRE(client.connect0Rtt(info).ok());
    REQUIRE(PumpUntil(loop, [&]() { return rejected; }, nullptr, 300, 1));
    REQUIRE_FALSE(newConnectionCalled);
...
TEST_CASE("Context integration: connect0RttWithState succeeds after phase1 close", "[Context][Integration][0RTT][ResumptionState]")
{
    ...
    phase1.timeout = 300;
    phase1.encrypted = Context::kEncryptionNone;
...
TEST_CASE("Context integration: pending buffers replay once after delayed HandshakeDone", "[Context][Integration][Handshake][Regression]")
{
    ...
    auto deliverToPendingPath = [&](const std::vector<uint8_t> &packetBytes) {
        ...
        if (!decoded->hasFrame(eular::utp::kFrameHandshakeDone)) {
            AppendBufferedPendingPacket(pendingIt->second, packetBytes);
            return;
        }
        ...
        for (const auto &cached : snapshot.bufferedBeforeHandshakeDone) {
            ...
            REQUIRE(static_cast<size_t>(cached.offset) + cached.len <=
                    snapshot.bufferedBeforeHandshakeDoneStorage.size());
            replay.data = snapshot.bufferedBeforeHandshakeDoneStorage.data() + cached.offset;
            replay.len = cached.len;
    ...
    const uint64_t bytesInBeforeReplay = connIt->second->m_bytesIn;
    UdpSocket::MsgMetaInfo duplicateMsg{};
    duplicateMsg.data = (void*)packetADuplicate.data();
    duplicateMsg.len = packetADuplicate.size();
    duplicateMsg.metaInfo.peerAddress = peerAddr;
    connIt->second->onUdpPacket(duplicateMsg);
    REQUIRE(connIt->second->m_bytesIn == bytesInBeforeReplay);
    REQUIRE(connIt->second->m_ackElicitingSinceLastAck == 0);
          ...
          "[Context][Integration][Handshake][Regression]")
{
    ...
    auto handlePendingPacket = [&]() {
        ...
        REQUIRE(conn->m_packetNumber == 78);
        REQUIRE(conn->m_sendCtl->largestSentPacketNo() == 77);
        ctx.removePendingIncoming(localCid);
    };
...
TEST_CASE("Context integration: stream OnWritable fires again after queued data drains", "[Context][Integration][Stream]")
{
    ...
    clientConn->flushPendingStreamWrites(eular::utp::time::MonotonicUs());
