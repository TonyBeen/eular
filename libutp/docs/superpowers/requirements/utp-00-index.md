...
- 当前实现以 `c/` C11 源码和 `doc/` 模块文档为准。本目录的 12 份需求保留为迁移过程和协议决策参考，不能作为当前 C API、配置或实现状态的直接说明。
...
> 维护边界：本目录中的部分需求文档来自旧 C++ 实现迁移过程。当前实现审核和公共行为以 `c/include/utp/`、`c/src/` 及 `doc/` 为准；文档中的 `cpp/` 引用仅表示历史来源，不能作为当前 API 或实现状态依据。
