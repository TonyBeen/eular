...
### 10.1 #9 定稿:Finished 在打洞下的传输行为

**边界:本 spec 只定 Finished 的传输行为(可靠/路由/cid 归并/清零时机);其密码学内容(HMAC、transcript、key confirmation 语义、抗 MITM、密钥派生)归 crypto spec。**

1. **可靠重传**:Finished 是可靠帧(有包号、要 ACK),丢了重传到对端 ACK;**`connected` 不清空未确认 Finished 的重传队列**(同 §4.3 不变量)。重传耗尽 → 该路径失败。
2. **加密模式 connected 以 Finished 为准**:不同于明文"收到任一有效回包即 connected"(§4.3),**加密模式必须验证对端 Finished 才 connected**(先确认密钥再交付数据)。双向 Finished 均可靠 + 互确认 → 收敛,不 split-brain。
3. **Finished 只在胜出/已提交 route 上发,不 spray 到所有候选**:握手在多候选竞速,**首个到"可发 Finished"状态的路径提交为 route**,Finished 只走它,其余候选取消(§6.8)。避免多路完成、浪费。
4. **committed route 的 Finished 完不成 → 迁移**:已提交路径 Finished 重传耗尽,若仍在竞速窗口内 fail over 到次优候选、在新路径重试;都失败则连接失败。
5. **双方 Finished 按 cid 归并**:FinishedA/FinishedB 带 cid,交叉/重复按 cid + 包号去重(§8),不新建连接。
6. **密钥材料清零**:握手失败/被拒/取消/超时/关闭的所有路径,临时私钥、shared secret、Finished key、中间 HKDF 输出必须清零。
...
**核心机制:有界发送始终在线(不因 token 拒绝任何请求),token 只提升信任。**

- **握手很小 → 数据量限制不伤连接**:一次打洞握手只需几个小包(Initial + 少量重传 + PATH_CHALLENGE)。给"未验证地址"设一个**够握手用的发送预算**,合法连接(含 CGNAT/多线)照样完成,却把伪造反射封在几 KB。
- **永不因 token 拒绝**:签名无效、IP 不匹配(CGNAT/多线路,源 IP 可能 ≠ 注册时 IP)一律**不拒**,只保持有界发送。

**分级(都不拒绝)**

| 请求 | NtrsB 处理 | B 认知 / 发送 |
|---|---|---|
| **有效 token** | 转发,附"pid 已验证";按 **pid** 限速 | **知道是真实 A**,正常发(握手量够用) |
| 无 token / 签名无效 / IP 不符 | 不拒,转发但标"未验证";按**源 IP** 限速 | 有界发送,对端不响应即停 |

- **token**:A 的 home NtrsA 在注册/NAT 探测(§4.1)时签发,集群共享信任根,`{pid, 过期时间, 签名}`(**不硬绑 IP**,避免 CGNAT/多线误判;IP 至多作软信息)。**NtrsB 验签,把结论随转发告诉 B**(走可信 keepalive),B 信 NtrsB 结论,自己不验。

**三道叠加**
- **M1**:CONNECT 填充到 ≥ NtrsB 回复大小 → off-NtrsB 放大 **≤1**。
- **M2**:向未验证地址**有界发送**(预算够握手、对端不响应即停;验证后解锁激进重传) → off-B 反射封顶几 KB。
...
**边界(非本期)**:跨运营商 NtrsA/NtrsB 不共享信任根 → 无法验签 → 该请求走"未验证"档(仍可连,更严限速)。token 字段/签名算法随 crypto 细节最终定。
...
已定稿:**#1(方向/cid 归并/反向提升)、#2(对称/回观测源)、#3(单 cid 去重)、#12(host-local/hairpinning,见 §6.6)、#4/#5(反射/放大缓解,见 §12)、#8(MTU/候选上限,见 §6.7)、#7(取消/半开清理/资源上限,见 §6.8)、#9(Finished 传输行为,见 §10.1)**。
...
| 9 | 加密 Finished 在打洞丢包下的重传;双方都发时绑提交路由 | 健壮性 | ✅ 定稿 §10.1 |
| 10 | keepalive 间隔 < NAT 映射超时(默认可配) | 运维(连上后掉线) | 待解 |
| 11 | 版本协商:新类型/帧不识别时优雅降级 | 兼容 | 待解 |

下一步优先:**#10(keepalive 间隔 < NAT 超时)**,随后 #6(0-RTT 重放契约)、#11(版本协商)。
