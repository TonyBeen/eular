...
6. **失效恢复(P2P 特有)**:映射失效/网络变化时**不直接断**——优先在其余候选上 PATH 重验证(迁移),或**经 NTRS 重新 CONNECT 打洞**恢复,利用缓存/上报候选保住逻辑连接;都不行再关闭。
...
5. **按 cid 归并**:HandshakeDone(沿用现有单/双向语义)带 cid,交叉/重复按 cid + 包号去重(§8),不新建连接。
...
已定稿:**#1(方向/cid 归并/反向提升)、#2(对称/回观测源)、#3(单 cid 去重)、#12(host-local/hairpinning,见 §6.6)、#4/#5(反射/放大缓解,见 §12)、#8(MTU/候选上限,见 §6.7)、#7(取消/半开清理/资源上限,见 §6.8)、#9(HandshakeDone 传输行为,见 §10.1)、#10(保活/映射维持,见 §6.9)**。
