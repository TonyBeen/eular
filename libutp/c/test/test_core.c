...
typedef struct log_capture {
    ...
    char            message[UTP_LOG_MESSAGE_MAX_LENGTH + 1u];
} log_capture_t;
...
static void test_version_and_status(void) {
    ...
    assert(UTP_STATUS_SOCKET_OPEN == -0x0020);
    assert(UTP_STATUS_SOCKET_READ < -0x0020 && UTP_STATUS_SOCKET_READ > -0x0040);
    assert(UTP_STATUS_STREAM_FLOW_CONTROL <= -0x0040 && UTP_STATUS_STREAM_FLOW_CONTROL > -0x0060);
    assert(UTP_STATUS_CONNECTION_HANDSHAKE <= -0x00c0 && UTP_STATUS_CONNECTION_HANDSHAKE > -0x00e0);
    assert(strcmp(utp_status_string(UTP_STATUS_SOCKET_READ), "socket_read") == 0);
...
static void test_buffer(void) {
    ...
    assert(utp_buffer_append(&buffer, first, sizeof(first) - 1u) == UTP_INTERNAL_ERROR_OK);
    assert(utp_buffer_append(&buffer, second, sizeof(second) - 1u) == UTP_INTERNAL_ERROR_OK);
    ...
    assert(utp_buffer_append(&buffer, "0123456789", 10u) == UTP_INTERNAL_ERROR_LIMIT);
...
static void test_range_set(void) {
    ...
    assert(utp_range_set_insert(&set, 9u, 9u) == UTP_INTERNAL_ERROR_INVALID_ARGUMENT);
    utp_range_set_cleanup(&set);
    utp_range_set_cleanup(&set);

    assert(utp_range_set_init(&single_range_set, NULL, 1u) == UTP_INTERNAL_ERROR_OK);
    assert(utp_range_set_insert(&single_range_set, 1u, 2u) == UTP_INTERNAL_ERROR_OK);
    assert(single_range_set.capacity == 1u);
    assert(utp_range_set_insert(&single_range_set, 3u, 4u) == UTP_INTERNAL_ERROR_LIMIT);
...
static void test_range_set_allocation_failure(void) {
    ...
    for (index = 0; index < 8u; ++index) {
        ...
        assert(utp_range_set_insert(&set, start, start + 1u) == UTP_INTERNAL_ERROR_OK);
    }
    ...
    assert(set.ranges == saved_ranges && set.count == 8u && set.capacity == saved_capacity);
    for (index = 0; index < 8u; ++index) {
...
static void test_hash(void) {
    ...
    assert(utp_hash_table_init(&table, NULL, ITEM_COUNT) == UTP_INTERNAL_ERROR_OK);
    for (index = 0; index < ITEM_COUNT; ++index) {
        items[index].key = (uint32_t)index;
        utp_hash_node_init(&items[index].node);
        assert(utp_hash_table_insert(&table, &items[index].node, hash_key(items[index].key), &items[index].key,
                                     hash_matches, NULL) == UTP_INTERNAL_ERROR_OK);
        items[index].present = 1;
    }
    assert(utp_hash_table_count(&table) == ITEM_COUNT);
    assert(utp_hash_table_insert(&table, &items[0].node, hash_key(items[0].key), &items[0].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_STATE);
    duplicate.key = items[0].key;
    utp_hash_node_init(&duplicate.node);
    assert(utp_hash_table_insert(&table, &duplicate.node, hash_key(duplicate.key), &duplicate.key, hash_matches,
                                 NULL) == UTP_INTERNAL_ERROR_EXISTS);
    assert(duplicate.node.prev_next == NULL && duplicate.node.table == NULL);
    for (index = 0; index < ITEM_COUNT; ++index) {
        node = utp_hash_table_find(&table, hash_key((uint32_t)index), &items[index].key, hash_matches, NULL);
    ...
    for (index = 0; index < ITEM_COUNT; index += 2u) {
        assert(utp_hash_table_remove(&table, &items[index].node) == UTP_INTERNAL_ERROR_OK);
        items[index].present = 0;
    }
    for (index = 0; index < ITEM_COUNT; ++index) {
        node = utp_hash_table_find(&table, hash_key(items[index].key), &items[index].key, hash_matches, NULL);
...
static void test_hash_capacity_and_table_ownership(void) {
    ...
    assert(utp_hash_table_insert(&first, &items[0].node, hash_key(items[0].key), &items[0].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_OK);
    assert(utp_hash_table_insert(&first, &items[1].node, hash_key(items[1].key), &items[1].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_OK);
    assert(utp_hash_table_insert(&first, &items[2].node, hash_key(items[2].key), &items[2].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_LIMIT);
    assert(utp_hash_table_insert(&second, &items[0].node, hash_key(items[0].key), &items[0].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_STATE);
    assert(utp_hash_table_remove(&second, &items[0].node) == UTP_INTERNAL_ERROR_NOT_FOUND);
    assert(utp_hash_table_count(&first) == 2u && utp_hash_table_count(&second) == 0u);
    assert(utp_hash_table_remove(&first, &items[0].node) == UTP_INTERNAL_ERROR_OK);
    assert(utp_hash_table_insert(&second, &items[0].node, hash_key(items[0].key), &items[0].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_OK);
...
static void test_hash_initial_bucket_rounding(void) {
    ...
    assert(utp_hash_table_init_with_buckets(&table, NULL, 64u, 9u) == UTP_INTERNAL_ERROR_OK);
    assert(table.bucket_count == 16u && table.buckets != NULL);
    utp_hash_table_cleanup(&table, NULL, NULL);

    assert(utp_hash_table_init_with_buckets(&table, NULL, 64u, 1u) == UTP_INTERNAL_ERROR_OK);
    assert(table.bucket_count == 1u && table.buckets != NULL);
    utp_hash_table_cleanup(&table, NULL, NULL);

    assert(utp_hash_table_init_with_buckets(&table, NULL, 64u, SIZE_MAX) == UTP_INTERNAL_ERROR_OVERFLOW);
    assert(table.buckets == NULL && table.bucket_count == 0u && table.allocator == NULL);

    assert(utp_hash_table_init_with_buckets(&table, &failed_allocator, 64u, 9u) == UTP_INTERNAL_ERROR_NOMEM);
    assert(table.buckets == NULL && table.bucket_count == 0u && table.allocator == NULL);
    utp_hash_table_cleanup(&table, NULL, NULL);
}
...
static void test_hash_random(void) {
    ...
    for (operation = 0; operation < OPERATIONS; ++operation) {
        ...
        if (action == 0u) {
            ...
            assert(status == (items[key].present ? UTP_INTERNAL_ERROR_STATE : UTP_INTERNAL_ERROR_OK));
            if (status == UTP_INTERNAL_ERROR_OK) {
                items[key].present = 1;
            }
        } else if (action == 1u) {
            utp_internal_error_t status = utp_hash_table_remove(&table, &items[key].node);
            assert(status == (items[key].present ? UTP_INTERNAL_ERROR_OK : UTP_INTERNAL_ERROR_NOT_FOUND));
            if (status == UTP_INTERNAL_ERROR_OK) {
                items[key].present = 0;
            }
        } else {
...
static void test_hash_allocation_failure(void) {
    ...
    assert(utp_hash_table_insert(&table, &item.node, hash_key(key), &key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_NOMEM);
    assert(table.count == 0u && table.buckets == NULL && item.node.prev_next == NULL);
    state.calls      = 0u;
    state.fail_after = 1u;
    assert(utp_hash_table_insert(&table, &item.node, hash_key(key), &key, hash_matches, NULL) == UTP_INTERNAL_ERROR_OK);
...
static void test_hash_rehash_allocation_failure(void) {
    ...
    state.fail_after = state.calls;
    assert(utp_hash_table_insert(&table, &items[6].node, hash_key(items[6].key), &items[6].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_NOMEM);
    assert(table.bucket_count == 8u && table.count == 6u && items[6].node.table == NULL);
    for (index = 0; index < 6u; ++index) {
        uint32_t key = (uint32_t)index;
        assert(utp_hash_table_find(&table, hash_key(key), &key, hash_matches, NULL) == &items[index].node);
    }
    state.fail_after = SIZE_MAX;
    assert(utp_hash_table_insert(&table, &items[6].node, hash_key(items[6].key), &items[6].key, hash_matches, NULL) ==
           UTP_INTERNAL_ERROR_OK);
