...
struct test_nat_server {
    ...
    std::array<test_nat_server_socket, 4u> sockets                   = {};
    std::array<event*, 4u>                 events                    = {};
    std::array<utp_address_t, 4u>          endpoints                 = {};
    std::array<size_t, 3u>                 request_counts            = {};
    std::array<uint8_t, 3u>                ignored_response_masks    = {};
    utp_address_t                          mapped_endpoint           = {};
    utp_address_t                          alternate_mapped_endpoint = {};
};
...
enum : uint8_t {
    ...
    TEST_NAT_SERVER_ALTERNATE_CHANGE_PORT,
};
...
static bool test_nat_server_parse_request(const uint8_t* packet, size_t packet_length, utp_packet_header_t* header,
                                          uint8_t* step, uint8_t* change_flags, const uint8_t** token)
{
    ...
    if (utp_proto_decode_header(header, packet, packet_length) != UTP_INTERNAL_ERROR_OK ||
        ...
        utp_wire_read_u8(&reader, step) != UTP_INTERNAL_ERROR_OK ||
        utp_wire_read_u8(&reader, change_flags) != UTP_INTERNAL_ERROR_OK || version != UTP_NAT_PROBE_VERSION ||
        message_type != UTP_NAT_PROBE_MESSAGE_BINDING_REQUEST ||
        ((*step == UTP_NAT_PROBE_STEP_PRIMARY_BINDING && *change_flags != UTP_NAT_PROBE_CHANGE_BOTH) ||
         (*step == UTP_NAT_PROBE_STEP_ALTERNATE_BINDING && *change_flags != UTP_NAT_PROBE_CHANGE_PORT))) {
        return false;
    }
...
static bool test_nat_server_build_response(const test_nat_server& server, const utp_packet_header_t& request_header,
                                           uint8_t step, uint8_t change_flags, const uint8_t* token,
                                           const utp_address_t& client, uint8_t response_role,
                                           uint8_t response[UTP_NAT_PROBE_PACKET_SIZE], size_t* response_length)
{
    ...
    mapped               = step == UTP_NAT_PROBE_STEP_ALTERNATE_BINDING && server.alternate_mapped_endpoint.family != 0u
                               ? server.alternate_mapped_endpoint
                               : server.mapped_endpoint;
    if (mapped.port == 0u) {
    ...
    if (utp_wire_writer_init(&writer, response + UTP_PACKET_HEADER_SIZE,
        ...
        utp_wire_write_u8(&writer, UTP_NAT_PROBE_MESSAGE_BINDING_RESPONSE) != UTP_INTERNAL_ERROR_OK ||
        utp_wire_write_u8(&writer, step) != UTP_INTERNAL_ERROR_OK ||
        utp_wire_write_u8(&writer, change_flags) != UTP_INTERNAL_ERROR_OK) {
        return false;
    }
    ...
    if (step == UTP_NAT_PROBE_STEP_PRIMARY_BINDING) {
...
static void test_nat_server_on_read(evutil_socket_t fd, int16_t events, void* user_data)
{
    ...
    if (received <= 0 || peer.ss_family != AF_INET ||
        !test_nat_server_parse_request(request.data(), static_cast<size_t>(received), &header, &step, &change_flags,
                                       &token)) {
        return;
    }
    if (step < socket->server->request_counts.size()) {
        ++socket->server->request_counts[step];
    }
    ...
    const std::array<std::pair<uint8_t, uint8_t>, 2u> primary_responses = {
        std::pair<uint8_t, uint8_t>{UTP_NAT_PROBE_CHANGE_NONE, TEST_NAT_SERVER_PRIMARY},
        std::pair<uint8_t, uint8_t>{UTP_NAT_PROBE_CHANGE_BOTH, TEST_NAT_SERVER_ALTERNATE_CHANGE_PORT},
    };
    const std::array<std::pair<uint8_t, uint8_t>, 2u> alternate_responses = {
        std::pair<uint8_t, uint8_t>{UTP_NAT_PROBE_CHANGE_NONE, TEST_NAT_SERVER_ALTERNATE},
        std::pair<uint8_t, uint8_t>{UTP_NAT_PROBE_CHANGE_PORT, TEST_NAT_SERVER_ALTERNATE_CHANGE_PORT},
    };
    const auto& responses = step == UTP_NAT_PROBE_STEP_PRIMARY_BINDING ? primary_responses : alternate_responses;
    for (const auto& item : responses) {
        const uint8_t response_mask = item.first == UTP_NAT_PROBE_CHANGE_NONE   ? UTP_NAT_PROBE_RESPONSE_PRIMARY
                                      : item.first == UTP_NAT_PROBE_CHANGE_PORT ? UTP_NAT_PROBE_RESPONSE_CHANGE_PORT
                                                                                : UTP_NAT_PROBE_RESPONSE_CHANGE_IP_PORT;

        if ((socket->server->ignored_response_masks[step] & response_mask) == 0u &&
            test_nat_server_build_response(*socket->server, header, step, item.first, token, client, item.second,
                                           response.data(), &response_length)) {
            (void)sendto(socket->server->sockets[item.second].fd, response.data(), response_length, 0,
...
{
    const std::array<uint8_t, 4u> octets = {1u, 1u, 2u, 2u};
...
TEST_CASE("nat primary binding request has a fixed UTP packet layout", "[nat]")
{
    ...
    REQUIRE(utp_nat_probe_encode_request(packet.data(), 42u, UTP_NAT_PROBE_STEP_PRIMARY_BINDING, token.data()) ==
            UTP_INTERNAL_ERROR_OK);
    ...
    REQUIRE(packet[UTP_PACKET_HEADER_SIZE + 1u] == UTP_NAT_PROBE_MESSAGE_BINDING_REQUEST);
    REQUIRE(packet[UTP_PACKET_HEADER_SIZE + 2u] == UTP_NAT_PROBE_STEP_PRIMARY_BINDING);
    REQUIRE(packet[UTP_PACKET_HEADER_SIZE + 3u] == UTP_NAT_PROBE_CHANGE_BOTH);
    REQUIRE(utp_nat_probe_encode_request(packet.data(), 0u, UTP_NAT_PROBE_STEP_PRIMARY_BINDING, token.data()) ==
            UTP_INTERNAL_ERROR_INVALID_ARGUMENT);
    REQUIRE(utp_nat_probe_encode_request(packet.data(), 42u, UTP_NAT_PROBE_STEP_ALTERNATE_BINDING, token.data()) ==
            UTP_INTERNAL_ERROR_OK);
    REQUIRE(packet[UTP_PACKET_HEADER_SIZE + 3u] == UTP_NAT_PROBE_CHANGE_PORT);
}
...
TEST_CASE("nat probe response decodes mandatory endpoint fields", "[nat]")
{
    ...
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_MESSAGE_BINDING_RESPONSE) == UTP_INTERNAL_ERROR_OK);
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_STEP_PRIMARY_BINDING) == UTP_INTERNAL_ERROR_OK);
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_CHANGE_BOTH) == UTP_INTERNAL_ERROR_OK);
    ...
    REQUIRE(response.message_type == UTP_NAT_PROBE_MESSAGE_BINDING_RESPONSE);
    REQUIRE(response.step == UTP_NAT_PROBE_STEP_PRIMARY_BINDING);
    REQUIRE(response.change_flags == UTP_NAT_PROBE_CHANGE_BOTH);
...
TEST_CASE("nat probe response only permits alternate endpoint during primary binding", "[nat]")
{
    const std::array<uint8_t, UTP_NAT_PROBE_TOKEN_SIZE> token = {
    ...
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_VERSION) == UTP_INTERNAL_ERROR_OK);
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_MESSAGE_BINDING_RESPONSE) == UTP_INTERNAL_ERROR_OK);
    REQUIRE(utp_wire_write_u8(&writer, UTP_NAT_PROBE_STEP_ALTERNATE_BINDING) == UTP_INTERNAL_ERROR_OK);
    REQUIRE(utp_wire_write_u8(&writer, 0u) == UTP_INTERNAL_ERROR_OK);
    test_nat_write_tlv(&writer, UTP_NAT_PROBE_TLV_PROBE_TOKEN, token.data(), (uint16_t)token.size());
    test_nat_write_tlv(&writer, UTP_NAT_PROBE_TLV_MAPPED_ADDR, endpoint.data(), (uint16_t)endpoint.size());
    test_nat_write_tlv(&writer, UTP_NAT_PROBE_TLV_ORIGIN_ADDR, endpoint.data(), (uint16_t)endpoint.size());
...
TEST_CASE("nat probe completes full cone detection during primary binding", "[nat][context]")
{
    event_base*           event_base      = event_base_new();
    utp_context_options_t context_options = UTP_CONTEXT_OPTIONS_INIT;
    context_options.peer_id               = "test";
...
TEST_CASE("nat probe falls back to alternate binding without a combined response", "[nat][context]")
{
    event_base*           event_base      = event_base_new();
    utp_context_options_t context_options = UTP_CONTEXT_OPTIONS_INIT;
    context_options.peer_id               = "test";
    ...
    REQUIRE(event_base != nullptr);
    REQUIRE(test_nat_server_start(&server, event_base));
    server.ignored_response_masks[UTP_NAT_PROBE_STEP_PRIMARY_BINDING] = UTP_NAT_PROBE_RESPONSE_CHANGE_IP_PORT;
    context_options.event_base                                        = event_base;
    REQUIRE(utp_context_create(&context_options, &context) == UTP_STATUS_OK);
    REQUIRE(utp_context_bind(context, "127.0.0.1", 0u, nullptr, nullptr) == UTP_STATUS_OK);
    probe_options.nat_service_address = "127.0.0.1";
    probe_options.nat_service_port    = server.endpoints[TEST_NAT_SERVER_PRIMARY].port;
    probe_options.phase_timeout_ms    = 60u;
    REQUIRE(utp_context_probe_nat(context, &probe_options, test_nat_probe_complete, &callback) == UTP_STATUS_OK);
    for (uint16_t index = 0u; index < 128u && callback.calls == 0u; ++index) {
        REQUIRE(event_base_loop(event_base, EVLOOP_ONCE) == 0);
    }
    REQUIRE(callback.calls == 1u);
    REQUIRE(callback.status == UTP_STATUS_OK);
    REQUIRE(callback.has_result);
    REQUIRE(callback.result.nat_class == UTP_NAT_CLASS_IP_RESTRICTED);
    REQUIRE(callback.result.primary_mapped_endpoint.family == UTP_ADDRESS_FAMILY_IPV4);
    REQUIRE(callback.result.secondary_mapped_endpoint.family == UTP_ADDRESS_FAMILY_IPV4);
    REQUIRE(server.request_counts[UTP_NAT_PROBE_STEP_PRIMARY_BINDING] != 0u);
    REQUIRE(server.request_counts[UTP_NAT_PROBE_STEP_ALTERNATE_BINDING] != 0u);
    utp_context_destroy(context);
    test_nat_server_stop(&server);
...
TEST_CASE("nat alternate binding preserves filtering and mapping classifications", "[nat][context]")
{
    event_base*           event_base       = event_base_new();
    utp_context_options_t context_options  = UTP_CONTEXT_OPTIONS_INIT;
    context_options.peer_id                = "test";
    ...
    test_nat_probe_callback callback       = {};
    test_nat_server         server         = {};
    utp_nat_class_t         expected_class = UTP_NAT_CLASS_UNKNOWN;

    REQUIRE(event_base != nullptr);
    REQUIRE(test_nat_server_start(&server, event_base));
    server.ignored_response_masks[UTP_NAT_PROBE_STEP_PRIMARY_BINDING] = UTP_NAT_PROBE_RESPONSE_CHANGE_IP_PORT;
    SECTION("missing change-port response is port restricted")
    {
        server.ignored_response_masks[UTP_NAT_PROBE_STEP_ALTERNATE_BINDING] = UTP_NAT_PROBE_RESPONSE_CHANGE_PORT;
        expected_class                                                      = UTP_NAT_CLASS_PORT_RESTRICTED;
    }
    SECTION("changed public port is symmetric")
    {
        server.alternate_mapped_endpoint      = server.mapped_endpoint;
        server.alternate_mapped_endpoint.port = 50000u;
        expected_class                        = UTP_NAT_CLASS_SYMMETRIC;
    }
    SECTION("changed public IP is symmetric multi-line")
    {
        server.alternate_mapped_endpoint            = server.mapped_endpoint;
        server.alternate_mapped_endpoint.address[0] = 203u;
        server.alternate_mapped_endpoint.address[1] = 0u;
        server.alternate_mapped_endpoint.address[2] = 113u;
        server.alternate_mapped_endpoint.address[3] = 7u;
        expected_class                              = UTP_NAT_CLASS_SYMMETRIC_MULTI_LINE;
    }
    SECTION("missing alternate mapping remains unknown")
    {
        server.ignored_response_masks[UTP_NAT_PROBE_STEP_ALTERNATE_BINDING] = UTP_NAT_PROBE_RESPONSE_PRIMARY;
        expected_class                                                      = UTP_NAT_CLASS_UNKNOWN;
    }
    context_options.event_base = event_base;
    REQUIRE(utp_context_create(&context_options, &context) == UTP_STATUS_OK);
    REQUIRE(utp_context_bind(context, "127.0.0.1", 0u, nullptr, nullptr) == UTP_STATUS_OK);
...
TEST_CASE("wildcard bound public endpoint without filtering evidence is classified with firewall", "[nat][context]")
{
    event_base*           event_base      = event_base_new();
    utp_context_options_t context_options = UTP_CONTEXT_OPTIONS_INIT;
    context_options.peer_id               = "test";
    ...
    server.mapped_endpoint.address[3]                                 = 1u;
    server.ignored_response_masks[UTP_NAT_PROBE_STEP_PRIMARY_BINDING] = UTP_NAT_PROBE_RESPONSE_CHANGE_IP_PORT;
    server.ignored_response_masks[UTP_NAT_PROBE_STEP_ALTERNATE_BINDING] =
        UTP_NAT_PROBE_RESPONSE_PRIMARY | UTP_NAT_PROBE_RESPONSE_CHANGE_PORT;
    context_options.event_base = event_base;
    REQUIRE(utp_context_create(&context_options, &context) == UTP_STATUS_OK);
    REQUIRE(utp_context_bind(context, "0.0.0.0", 0u, nullptr, nullptr) == UTP_STATUS_OK);
    probe_options.nat_service_address = "127.0.0.1";
    ...
    REQUIRE(callback.status == UTP_STATUS_OK);
    REQUIRE(callback.has_result);
    REQUIRE(callback.result.nat_class == UTP_NAT_CLASS_OPEN_PUBLIC_WITH_FIREWALL);
    utp_context_destroy(context);
    test_nat_server_stop(&server);
    event_base_free(event_base);
}
