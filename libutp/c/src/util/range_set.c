...
static utp_internal_error_t reserve_ranges(utp_range_set_t* set, size_t required)
{
    ...
    size_t capacity = set->capacity == 0 ? (set->max_ranges < 8u ? set->max_ranges : 8u) : set->capacity;
    ...
    if (capacity < required || capacity > SIZE_MAX / sizeof(utp_range_t)) {
        return UTP_INTERNAL_ERROR_OVERFLOW;
    }
    utp_range_t* ranges = utp_allocator_realloc(set->allocator, set->ranges, capacity * sizeof(*ranges));
    if (ranges == NULL) {
...
utp_internal_error_t utp_range_set_insert(utp_range_set_t* set, uint64_t start, uint64_t end)
{
    ...
    size_t removed = after - first;
    if (removed == 0) {
        if (set->count >= set->max_ranges) {
            return UTP_INTERNAL_ERROR_LIMIT;
        }
        utp_internal_error_t status = reserve_ranges(set, set->count + 1u);
        if (status != UTP_INTERNAL_ERROR_OK) {
            return status;
        }
        if (first < set->count) {
            memmove(&set->ranges[first + 1u], &set->ranges[first], (set->count - first) * sizeof(*set->ranges));
        }
        ++set->count;
    } else if (removed > 1u) {
        memmove(&set->ranges[first + 1u], &set->ranges[after], (set->count - after) * sizeof(*set->ranges));
