...
utp_internal_error_t utp_event_loop_init(utp_event_loop_t *loop) {
    if (loop == NULL) {
        return UTP_INTERNAL_ERROR_INVALID_ARGUMENT;
    }
    loop->native_base = event_base_new();
    return loop->native_base == NULL ? UTP_INTERNAL_ERROR_NOMEM : UTP_INTERNAL_ERROR_OK;
}

void utp_event_loop_close(utp_event_loop_t *loop) {
    if (loop != NULL && loop->native_base != NULL) {
        event_base_free(loop->native_base);
...
utp_internal_error_t utp_event_loop_run_once(utp_event_loop_t *loop, bool nonblocking) {
    ...
    return event_base_loop(loop->native_base, flags) < 0 ? UTP_INTERNAL_ERROR_IO : UTP_INTERNAL_ERROR_OK;
}
