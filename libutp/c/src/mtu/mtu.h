...
typedef enum utp_mtu_probe_state {
    UTP_MTU_PROBE_STATE_IDLE = 0,
    UTP_MTU_PROBE_STATE_QUEUED,
    UTP_MTU_PROBE_STATE_IN_FLIGHT
} utp_mtu_probe_state_t;
...
typedef struct utp_mtu_discovery {
    ...
    uint64_t              probe_deadline_ms;            // 已发送探测的超时截止时刻
    uint64_t              probe_packet_number;          // 已排队或已发送的探测包号
    ...
    uint16_t              probe_mtu;                    // 已排队或已发送的探测 MTU
    ...
    utp_mtu_probe_state_t probe_state;                  // 探测包发送生命周期
} utp_mtu_discovery_t;
