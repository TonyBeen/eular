...
void utp_mtu_discovery_init(utp_mtu_discovery_t* discovery, const utp_mtu_config_t* config, uint8_t family)
{
    ...
    discovery->blackhole_loss_threshold =
        config == NULL ? UTP_MTU_DEFAULT_BLACKHOLE_LOSS_THRESHOLD : config->blackhole_loss_threshold;
    discovery->blackhole_loss_window_ms =
        config == NULL ? UTP_MTU_DEFAULT_BLACKHOLE_LOSS_WINDOW_MS : config->blackhole_loss_window_ms;
    discovery->blackhole_cooldown_ms =
        config == NULL ? UTP_MTU_DEFAULT_BLACKHOLE_COOLDOWN_MS : config->blackhole_cooldown_ms;
    configured_interval_ms =
        config == NULL ? UTP_MTU_DEFAULT_PROBE_INTERVAL_MS : (uint64_t)config->probe_interval_seconds * UINT64_C(1000);
    discovery->probe_interval_ms           = configured_interval_ms == 0u          ? 0u
                                             : configured_interval_ms < 1000u      ? 1000u
