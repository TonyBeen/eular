...
int main(int argc, char **argv) {
    ...
    printf("libutp-c container benchmark: seed=0x%08" PRIx32 ", items=%u\n", seed, (unsigned int)ITEM_COUNT);
    start = clock();
    for (index = 0; index < ITEM_COUNT; ++index) {
        if (utp_hash_table_insert(&table, &items[index].node, hash_key(items[index].key), &items[index].key,
    ...
    start = clock();
    for (index = 0; index < ITEM_COUNT; ++index) {
        if (utp_hash_table_remove(&table, &items[index].node) != UTP_INTERNAL_ERROR_OK) {
    ...
    for (index = 0; index < operations; ++index) {
        ...
        if (utp_range_set_insert(&ranges, first, first + length) != UTP_INTERNAL_ERROR_OK) {
