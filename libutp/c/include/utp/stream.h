...
/** @brief 流发生本地或对端 RESET_STREAM 后同步调用，仅一次。 */
typedef void (*utp_on_stream_reset_fn)(utp_stream_t* stream, uint16_t error_code, void* user_data);
...
/** @brief 设置 RESET 回调；仅对后续 RESET 状态变化通知。 */
