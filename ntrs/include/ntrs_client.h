...
enum {
    ...
    NTRS_NAT_CLASS_IPV6_OPEN_PUBLIC = 6,
    NTRS_NAT_CLASS_IPV6_OPEN_PUBLIC_WITH_FIREWALL = 7,
    NTRS_NAT_CLASS_IPV6_UDP_BLOCKED = 8,
};
...
typedef struct ntrs_async_result {
     ...
     * @brief 私有探测主端点。
     *
     * 该字段是新的公共语义，对应私有 NAT 探测协议中的 probe1 端点。
     */
    char                      probe1[NTRS_MAX_TEXT_LEN];
    /**
     * @brief 私有探测辅助端点。
     *
     * 该字段是新的公共语义，对应私有 NAT 探测协议中的 probe2 端点。
     */
 ...
 * @brief 初始化 NAT 探测参数。
 *
 * @param options 待初始化参数。
 */
void ntrs_detect_nat_options_init(ntrs_detect_nat_options_t* options);

/**
 * @brief 建立到 NTRS 控制面的 TCP 连接。
 *
 * @param ntrs_ip 控制面地址。
 * @param ntrs_port 控制面端口。
 * @return int32_t 成功返回控制 socket，失败返回 -1。
 */
int32_t ntrs_connect_control(const char* ntrs_ip, uint16_t ntrs_port);

/**
 * @brief 向控制面请求私有 NAT 探测端点。
 *
 * 当前函数名仍保留历史兼容语义，但输出参数已经表示私有 `probe1/probe2`
 * 端点，而不是公开 NAT 探测服务器地址。
 *
 * @param control_fd 控制连接 socket。
 * @param session_token 会话令牌。
 * @param probe1 输出主探测端点。
 * @param probe1_len 主探测端点缓冲区长度。
 * @param probe2 输出辅助探测端点。
 * @param probe2_len 辅助探测端点缓冲区长度。
 * @return true 请求成功。
 * @return false 请求失败。
 */
...
/**
 * @brief 基于上层提供的 UDP socket 执行异步 NAT 探测。
 *
 * @param client 异步客户端。
 * @param request_id 输出请求编号。
 * @param udp_sock 上层已创建并绑定完成的 UDP socket。
 * @param probe1_host 私有主探测端点地址。
 * @param probe1_port 私有主探测端点端口。
 * @param probe2_host 私有辅助探测端点地址。
 * @param probe2_port 私有辅助探测端点端口。
 * @param control_fd 控制连接 socket。
 * @param session_token 会话令牌。
 * @param options 探测选项。
 * @param callback 完成回调。
